...
class LiquidityProber:
    ...
    async def _execute_probes(
        self, probe_tasks: list[tuple[float, asyncio.Task[float]]]
    ) -> list[tuple[float, float]]:
        """Execute all probe tasks and collect results."""
        results: list[tuple[float, float]] = []

        # Start all tasks (already scheduled); await results
        for price, task in probe_tasks:
            try:
    ...
    async def _single_probe(
        self, semaphore: asyncio.Semaphore, symbol: str, price: float, side: Literal["buy", "sell"]
    ) -> float:
        """
        ...
        async with semaphore:
            try:
                ...
                filled_volume = await self._wait_for_execution(
                    order_id, timeout=self.timeout_seconds
                )
    ...
    def calculate_liquidity_confidence_score(
        ...
        if len(probe_results) > 1:
            ...
            variance = sum((vol - avg_liquidity) ** 2 for vol in probe_results.values()) / len(
                probe_results
            )
