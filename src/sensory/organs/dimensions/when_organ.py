...
class WhenEngine:
    ...
    def __init__(self, config: Optional[dict[str, Any]] = None):
        ...
        self.temporal_analyzer: TemporalAnalyzer | None
        try:
    ...
    def _calculate_trend_strength(self, df: pd.DataFrame) -> float:
        ...
        try:
            ...
            x = np.arange(len(df))
            # Ensure a float ndarray to satisfy typing and numeric expectations
            y = np.asarray(df["close"].to_numpy(), dtype=float)
            ...
            ss_tot: float = float(np.sum((y - float(np.mean(y))) ** 2))
            r_squared = 1 - (ss_res / ss_tot) if ss_tot > 0 else 0

            trend_strength = abs(slope) * r_squared
            return float(min(max(float(trend_strength), 0.0), 1.0))
    ...
    def _calculate_signal_strength(self, analysis: dict[str, Any]) -> float:
        ...
        try:
            ...
            temporal_confidence = analysis.get("regime_detection", {}).get(
            ...
            signal_strength = (
                temporal_confidence * 0.4 + temporal_strength * 0.3 + timing_confidence * 0.3
            )
