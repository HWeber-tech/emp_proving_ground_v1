...
class CorrelationAnalyzer:
    ...
    def _calculate_dimensional_correlation(
        self, dim_a: str, dim_b: str
    ) -> Optional[DimensionalCorrelation]:
        ...
        for reading_a in history_a:
            ...
            for reading_b in history_b:
                # Within 5 minutes
                if abs((reading_b["timestamp"] - timestamp).total_seconds()) < 300:
        ...
        try:
            ...
            logger.warning(
                f"Failed to calculate correlation between {dim_a} and {dim_b}: {e}"
            )
            return None

    def _calculate_optimal_lag(
        ...
        # Maximum 5 periods or 25% of data
        max_lag = min(5, len(values_a) // 4)
    ...
    def _detect_fundamental_technical_confluence(
        self,
    ) -> Optional[CrossDimensionalPattern]:
        ...
        if why_direction == what_direction:
            ...
            if alignment_strength > 0.3:  # Significant alignment
                return CrossDimensionalPattern(
                    ...
                    confidence=0.7,
                    expected_outcome=(
                        "trend_continuation" if why_direction > 0 else "trend_reversal"
                    ),
    ...
    def _detect_institutional_temporal_alignment(
        self,
    ) -> Optional[CrossDimensionalPattern]:
    ...
    def _detect_multidimensional_breakout(self) -> Optional[CrossDimensionalPattern]:
        ...
        for dim in active_dimensions:
            ...
            avg_confidence = np.mean(
                [reading["confidence"] for reading in recent_readings]
            )

            if (
                abs(avg_value) > 0.3 and avg_confidence > 0.5
            ):  # Strong signal with confidence
    ...
    def get_dimensional_correlations(
        self,
    ) -> Dict[Tuple[str, str], DimensionalCorrelation]:
...
class AdaptiveWeightManager:
    ...
    def update_performance(self, dimension: str, accuracy: float) -> None:
        ...
        if dimension in self.weights:
            ...
            if len(self.performance_history[dimension]) > 20:
                self.performance_history[dimension] = self.performance_history[
                    dimension
                ][-20:]
            ...
            self.weights[dimension].performance_factor = self.prediction_accuracy[
                dimension
            ]
    ...
    def update_correlation_factors(
        self, correlations: Dict[Tuple[str, str], DimensionalCorrelation]
    ) -> None:
    ...
    def update_confidence_factors(
        self, readings: Dict[str, DimensionalReading]
    ) -> None:
...
class NarrativeGenerator:
    ...
    def _extract_evidence(
        ...
        for dimension, reading in readings.items():
            ...
            if dimension == "WHY":
                ...
                evidence["what_evidence"] = self._extract_what_evidence(
                    reading, context
                )
            elif dimension == "WHEN":
                evidence["when_evidence"] = self._extract_when_evidence(
                    reading, context
                )
            elif dimension == "ANOMALY":
                evidence["anomaly_evidence"] = self._extract_anomaly_evidence(
                    reading, context
                )
    ...
    def _extract_anomaly_evidence(
        self, reading: DimensionalReading, context: Dict
    ) -> str:
    ...
    def _construct_narrative(
        self, narrative_type: str, evidence: Dict[str, str], synthesis: MarketSynthesis
    ) -> str:
        """Construct narrative text using templates"""

        template_info = self.narrative_templates.get(
            narrative_type, self.narrative_templates["uncertainty"]
        )
    ...
    def _identify_conflicting_evidence(self, evidence: Dict[str, str]) -> str:
        ...
        for key, text in evidence.items():
            if any(
                word in text.lower()
                for word in ["positive", "strong", "supportive", "buying"]
            ):
                positive_indicators.append(key.replace("_evidence", ""))
            elif any(
                word in text.lower()
                for word in ["negative", "weak", "selling", "pressure"]
            ):
                negative_indicators.append(key.replace("_evidence", ""))

        if positive_indicators and negative_indicators:
            conflicts.append(
                f"{', '.join(positive_indicators)} showing strength while {', '.join(negative_indicators)} showing weakness"
            )
    ...
    def _add_correlation_insights(
        ...
        for corr in strong_correlations[:2]:  # Limit to top 2
            ...
            insights.append(
                f"{corr.dimension_a}-{corr.dimension_b} showing strong {direction} correlation"
            )
...
class ContextualFusionEngine:
    ...
    async def _get_dimensional_readings(
        self, market_data: MarketData
        ...
        try:
            # Get WHY reading
            why_reading = await self.why_engine.analyze_fundamental_intelligence(
                market_data
            )
        ...
        try:
            # Get HOW reading
            how_reading = await self.how_engine.analyze_institutional_mechanics(
                market_data
            )
        ...
        try:
            # Get WHEN reading
            when_reading = await self.when_engine.analyze_temporal_intelligence(
                market_data
            )
        ...
        try:
            # Get ANOMALY reading
            anomaly_reading = await self.anomaly_engine.analyze_anomaly_intelligence(
                market_data
            )
    ...
    def _calculate_unified_synthesis(
        self,
        readings: Dict[str, DimensionalReading],
        correlations: Dict[Tuple[str, str], DimensionalCorrelation],
        patterns: List[CrossDimensionalPattern],
        ...
        intelligence_level = self._determine_intelligence_level(
            readings, confidence, patterns
        )

        # Determine narrative coherence
        narrative_coherence = self._determine_narrative_coherence(
            readings, correlations
        )

        # Determine dominant narrative
        dominant_narrative = self._determine_dominant_narrative(
            readings, patterns, unified_score
        )

        # Extract supporting and contradicting evidence
        supporting_evidence, contradicting_evidence = self._extract_evidence_lists(
            readings, unified_score
        )

        # Identify risk and opportunity factors
        risk_factors, opportunity_factors = self._identify_risk_opportunity_factors(
            readings, patterns
        )
    ...
    def _calculate_overall_confidence(
        self, readings: Dict[str, DimensionalReading], weights: Dict[str, float]
    ) -> float:
        ...
        # Prefer all 5 dimensions
        data_quality_factor = min(len(readings) / 5.0, 1.0)

        final_confidence = (
            base_confidence * (1.0 + agreement_bonus * 0.2) * data_quality_factor
        )

        return min(final_confidence, 1.0)

    def _calculate_dimensional_agreement(
        self, readings: Dict[str, DimensionalReading]
    ) -> float:
    ...
    def _determine_narrative_coherence(
        self,
        readings: Dict[str, DimensionalReading],
        correlations: Dict[Tuple[str, str], DimensionalCorrelation],
    ) -> NarrativeCoherence:
        ...
        confidence_consistency = (
            1.0 - np.std(confidences) if len(confidences) > 1 else 1.0
        )
    ...
    def _extract_evidence_lists(
        self, readings: Dict[str, DimensionalReading], unified_score: float
    ...
    def _identify_risk_opportunity_factors(
        self,
        readings: Dict[str, DimensionalReading],
        patterns: List[CrossDimensionalPattern],
    ) -> Tuple[List[str], List[str]]:
        ...
        if "ANOMALY" in readings and readings["ANOMALY"].value > 0.4:
            ...
            if anomaly_context.get("system_stress_level", 0) > 0.5:
                risk_factors.append(
                    "High system stress indicates potential instability"
                )

        # Low confidence risks
        low_confidence_dimensions = [
            dim for dim, reading in readings.items() if reading.confidence < 0.4
        ]

        if len(low_confidence_dimensions) >= 2:
            risk_factors.append(
                f"Low confidence in {', '.join(low_confidence_dimensions)} dimensions"
            )
        ...
        for pattern in patterns:
            if pattern.confidence > 0.6:
                opportunity_factors.append(
                    f"{pattern.pattern_name.replace('_', ' ')} pattern with {pattern.confidence:.0%} confidence"
                )
        ...
        if len(high_confidence_dimensions) >= 2:
            opportunity_factors.append(
                f"High confidence signals from {', '.join(high_confidence_dimensions)}"
            )
        ...
        if "WHEN" in readings:
            ...
            if "OVERLAP" in when_context.get("current_session", ""):
                opportunity_factors.append(
                    "Session overlap provides increased volatility potential"
                )
        ...
        diagnostics = {
            ...
            "correlations": {
                ...
                for (
                    dim_a,
                    dim_b,
                ), corr in self.correlation_analyzer.get_dimensional_correlations().items()
            },
            "patterns": [
                {
    ...
    for i in range(100):
        ...
        if i % 20 == 0:  # Print every 20th analysis
            ...
            if synthesis.supporting_evidence:
                print(
                    f"  Supporting Evidence: {len(synthesis.supporting_evidence)} factors"
                )
            ...
            if synthesis.opportunity_factors:
                print(
                    f"  Opportunities: {len(synthesis.opportunity_factors)} identified"
                )
