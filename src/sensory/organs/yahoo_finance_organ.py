...
class YahooFinanceOrgan:
    ...
    def download_data(
        self, symbol: str, start_date: str, end_date: Optional[str] = None, interval: str = "1h"
    ) -> bool:
        """
    ...
    def fetch_data(
        ...
        try:
            ...
            data: pd.DataFrame = cast(
                pd.DataFrame, ticker.history(period=period, interval=interval)
            )
...
if __name__ == "__main__":
    ...
    success = organ.download_data(
        symbol="EURUSD", start_date="2024-01-01", end_date="2024-12-31", interval="1h"
    )
