...
- Progress: LOBSTER ingest adapter normalises message and order book files, scales prices, validates alignment, and ships regression coverage so microstructure analytics can consume high-frequency frames once reconstruction lands.【F:src/data_foundation/ingest/lobster_adapter.py†L1-L214】【F:src/data_foundation/ingest/lobster_adapter.py†L194-L214】【F:tests/data_foundation/test_lobster_adapter.py†L24-L103】
- Progress: Streaming market data cache now provides a Redis-compatible tick window with warm starts, TTL enforcement, and an in-memory fallback backed by pytest coverage, preparing the streaming pipeline for live adapters once broker feeds are wired.【F:src/data_foundation/streaming/market_cache.py†L1-L214】【F:src/data_foundation/streaming/market_cache.py†L160-L256】【F:tests/data_foundation/streaming/test_market_cache.py†L1-L68】
- Progress: A resilient WebSocket client with reconnect, heartbeat, token-bucket throttling, and durable subscription replay now ships alongside async pytest drills so upcoming venue adapters can stream ticks into the cache without rebuilding connectivity primitives or manual resubscription loops.【F:src/data_foundation/streaming/websocket_client.py†L62-L320】【F:tests/data_foundation/streaming/test_websocket_client.py†L209-L288】
...
- Progress: Operational backbone pipeline now threads adaptation feature toggles into belief snapshots, merging sensory `feature_flags` and fast-weight hints with environment defaults so ingest-driven adaptation honours tier/run-mode posture; the CLI resolves toggles from `SystemConfig` and regression coverage locks the merge helpers.【F:src/data_foundation/pipelines/operational_backbone.py†L35-L214】【F:src/data_foundation/pipelines/operational_backbone.py†L635-L665】【F:tools/data_ingest/run_operational_backbone.py†L287-L313】【F:tests/thinking/test_adaptation_feature_toggles.py†L1-L86】
...
- Progress: RealDataManager now detects institutional mode from `SystemConfig` or extras and defaults `require_timescale` accordingly, raising an explicit error when no Timescale connection is configured so institutional rehearsals cannot silently downgrade to SQLite; guardrails cover both the failure path and the configured success case.【F:src/data_integration/real_data_integration.py†L245-L299】【F:tests/data_integration/test_real_data_manager.py†L512-L528】
