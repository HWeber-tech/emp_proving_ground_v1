...
class CycleDetector(ThinkingPattern):
    ...
    def learn(self, feedback: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            if 'cycle_accuracy' in feedback:
                accuracy = feedback['cycle_accuracy']
    ...
    def _classify_cycle_type(self, values: List[float], peaks: List[int], troughs: List[int]) -> str:
    ...
    def _determine_cycle_phase(self, values: List[float], peaks: List[int], troughs: List[int]) -> str:
        ...
        if recent_peaks and recent_troughs:
            ...
            if last_peak > last_trough:
                ...
                phase_position = (current_index - last_peak) / (last_trough - last_peak) if last_trough > last_peak else 0.5
            else:
                # We're in a trough-to-peak phase
                phase_position = (current_index - last_trough) / (last_peak - last_trough) if last_peak > last_trough else 0.5
    ...
    def _calculate_cycle_confidence(self, values: List[float], peaks: List[int], troughs: List[int]) -> float:
        ...
        if len(peaks) >= 2 and len(troughs) >= 2:
            ...
            if peak_distances and trough_distances:
                peak_regularity = 1.0 - (np.std(peak_distances) / np.mean(peak_distances)) if np.mean(peak_distances) > 0 else 0.0
                trough_regularity = 1.0 - (np.std(trough_distances) / np.mean(trough_distances)) if np.mean(trough_distances) > 0 else 0.0
