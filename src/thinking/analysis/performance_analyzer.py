...
class PerformanceAnalyzer:
    ...
    def analyze_performance(
        self, trade_history: List[TradeIntent], initial_capital: float = 100000.0
    ) -> AnalysisResult:
        ...
        try:
            ...
            result = AnalysisResult(
                ...
                result={
                    ...
                    "final_capital": (
                        cast(pd.Series, performance_data["equity_curve"]).iloc[-1]
                        if len(cast(pd.Series, performance_data["equity_curve"])) > 0
            ...
            logger.debug(
                f"Performance analyzed: {metrics.total_return:.2%} return, {metrics.sharpe_ratio:.2f} Sharpe"
            )
            return result
    ...
    def analyze_backtest_results(self, backtest_results: dict[str, object]) -> AnalysisResult:
        ...
        try:
            ...
            # Use typed locals
            equity_curve_typed = cast(list[float], equity_curve)
            trades_typed = cast(list[dict[str, object]], trades)

            # Calculate returns from equity curve
            returns: list[float] = []
            for i in range(1, len(equity_curve_typed)):
                if equity_curve_typed[i - 1] > 0:
                    returns.append(
                        (equity_curve_typed[i] - equity_curve_typed[i - 1])
                        / equity_curve_typed[i - 1]
                    )

            # Create performance metrics
            metrics = self._calculate_metrics_from_returns(
                returns, equity_curve_typed, trades_typed
            )
            ...
            result = AnalysisResult(
                ...
                result={
                    "performance_metrics": metrics.__dict__,
                    "trade_count": len(trades_typed),
                    "equity_curve_length": len(equity_curve_typed),
                    "initial_capital": equity_curve_typed[0] if equity_curve_typed else 0,
                    "final_capital": equity_curve_typed[-1] if equity_curve_typed else 0,
                },
                confidence=confidence,
                metadata={
                    "analyzer_version": "1.1.0",
    ...
    def _calculate_metrics_from_returns(
        self, returns: list[float], equity_curve: list[float], trades: list[dict[str, object]]
    ) -> PerformanceMetrics:
        ...
        # Basic metrics
        total_return = (
            (equity_curve[-1] - equity_curve[0]) / equity_curve[0] if equity_curve[0] > 0 else 0
        )
        ...
        if volatility > 0:
            excess_return = (
                returns_array.mean() * self.trading_days_per_year
            ) - self.risk_free_rate
        ...
        if len(downside_returns) > 0:
            ...
            if downside_deviation > 0:
                sortino_ratio = (
                    (returns_array.mean() * self.trading_days_per_year) - self.risk_free_rate
                ) / downside_deviation
            else:
                sortino_ratio = 0
        else:
        ...
        # Maximum drawdown
        max_drawdown = self._calculate_max_drawdown(pd.Series(equity_curve))

        # Win rate and profit factor from trades
        if trades:
            winning_trades = [t for t in trades if float(cast(float, t.get("pnl", 0.0))) > 0.0]
            losing_trades = [t for t in trades if float(cast(float, t.get("pnl", 0.0))) < 0.0]

            win_rate = len(winning_trades) / len(trades)

            total_wins = sum(float(cast(float, t.get("pnl", 0.0))) for t in winning_trades)
            total_losses = abs(sum(float(cast(float, t.get("pnl", 0.0))) for t in losing_trades))
        ...
        return PerformanceMetrics(
            ...
            annualized_return=(
                total_return * (self.trading_days_per_year / len(returns)) if returns else 0
            ),
    ...
    def _convert_trades_to_performance(
        self, trade_history: List[TradeIntent], initial_capital: float
    ) -> dict[str, object]:
    ...
    def _calculate_performance_metrics(
        self, performance_data: dict[str, object]
    ) -> PerformanceMetrics:
        """Calculate comprehensive performance metrics."""
        equity_curve = cast(pd.Series, performance_data["equity_curve"])
        returns = cast(pd.Series, performance_data["returns"])
        trade_returns: list[float] = cast(list[float], performance_data["trade_returns"])
        initial_capital = float(cast(float, performance_data["initial_capital"]))
        final_capital = float(cast(float, performance_data["final_capital"]))
        ...
        # Annualized return
        if len(equity_curve) > 1:
            delta = cast(pd.Timedelta, equity_curve.index[-1] - equity_curve.index[0])
            days = int(delta.days)
            if days > 0:
    ...
    def _calculate_avg_trade_duration(self, performance_data: dict[str, object]) -> float:
        ...
        ec = cast(pd.Series, performance_data["equity_curve"])
        if len(ec) < 2:
            return 0.0

        total_days = int(cast(pd.Timedelta, ec.index[-1] - ec.index[0]).days)
        trade_returns_list = cast(list[float], performance_data["trade_returns"])
        trade_count = len(trade_returns_list)
    ...
    def _calculate_analysis_confidence(self, performance_data: dict[str, object]) -> float:
        ...
        ec = cast(pd.Series, performance_data["equity_curve"])
        tr = cast(list[float], performance_data["trade_returns"])
        ...
        # Time span
        if len(ec) > 1:
            span_delta = cast(pd.Timedelta, ec.index[-1] - ec.index[0])
            time_span = int(span_delta.days)
    ...
    def _calculate_backtest_confidence(self, backtest_results: dict[str, object]) -> float:
        ...
        # Data sufficiency
        equity_curve_list = cast(list[float], backtest_results.get("equity_curve", []))
        equity_curve_length = len(equity_curve_list)
        ...
        # Trade count
        trades_list = cast(list[dict[str, object]], backtest_results.get("trades", []))
    ...
    def get_performance_statistics(self) -> dict[str, object]:
        ...
        returns: list[float] = [cast(float, h["total_return"]) for h in self.analysis_history]
        sharpe_ratios: list[float] = [cast(float, h["sharpe_ratio"]) for h in self.analysis_history]
        drawdowns: list[float] = [cast(float, h["max_drawdown"]) for h in self.analysis_history]
        confidences: list[float] = [cast(float, h["confidence"]) for h in self.analysis_history]
