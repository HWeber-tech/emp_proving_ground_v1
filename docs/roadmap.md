...
- [ ] **Operational data backbone** – Deliver real Timescale/Redis/Kafka services,
  ...
  - *Progress*: Timescale reader guards schema/table/column identifiers, normalises
    ...
    covering identifier fuzzing.【F:src/data_foundation/persist/timescale_reader.py†L44-L395】【F:tests/data_foundation/test_timescale_reader_security.py†L1-L63】
- [ ] **Sensory + evolution execution** – Replace HOW/ANOMALY stubs, wire lineage
  ...
  - *Progress*: Ecosystem optimizer now defends against unsafe genomes and
    ...
    each guardrail so evolution runs cannot silently corrupt state.【F:src/ecosystem/optimization/ecosystem_optimizer.py†L104-L336】【F:src/ecosystem/optimization/ecosystem_optimizer.py†L503-L519】【F:tests/ecosystem/test_ecosystem_optimizer_hardening.py†L1-L104】
...
- [ ] **Dead code and duplication** – Triage the 168-file dead-code backlog and
  ...
  - *Progress*: Retired the legacy strategy template package and rewrote the
    ...
    production surface.【F:docs/reports/CLEANUP_REPORT.md†L80-L106】【F:tests/current/test_mean_reversion_strategy.py†L1-L108】
