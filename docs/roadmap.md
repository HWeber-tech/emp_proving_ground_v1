...
- [ ] `[obs]` Attribution coverage â‰¥ **90%** of orders have belief + probes; no Ïƒ explosions (bounded norms). _Progress: AlphaTradeLoopRunner now collapses rationale text into bounded `_build_brief_explanation` payloads and threads the trimmed attribution onto trade metadata, intents, outcomes, and loop metadata even when diaries are missing, stamping policy/diary ids when present so coverage snapshots stay in sync.【F:src/orchestration/alpha_trade_runner.py†L889】【F:src/orchestration/alpha_trade_runner.py†L267】【F:src/orchestration/alpha_trade_runner.py†L278】【F:src/orchestration/alpha_trade_runner.py†L331】【F:src/orchestration/alpha_trade_runner.py†L343】【F:src/orchestration/alpha_trade_runner.py†L915】【F:src/orchestration/alpha_trade_runner.py†L923】【F:src/orchestration/alpha_trade_runner.py†L925】 Diaries from 2025-10-20 (e.g., the replay `diary-011424` and counterfactual mirror `counterfactual_diary-010924`) show the recorded `brief_explanation`, policy ids, guardrail probes, and coverage snapshots under the new schema.【F:artifacts/diaries/2025/10/20/diary-011424/diary.json†L474】【F:artifacts/diaries/2025/10/20/diary-011424/diary.json†L687】【F:artifacts/diaries/2025/10/20/counterfactual_diary-010924/counterfactual_diary.json†L412】【F:artifacts/diaries/2025/10/20/counterfactual_diary-010924/counterfactual_diary.json†L635】 TradingManager normalises `brief_explanation` first, rehydrates attribution from trade outcomes when intents omit it, and keeps coverage counters warned when trimmed payloads are missing.【F:src/trading/trading_manager.py†L3947】【F:src/trading/trading_manager.py†L3953】【F:src/trading/trading_manager.py†L4160】【F:src/trading/trading_manager.py†L4166】【F:src/trading/trading_manager.py†L4180】 Orchestration and trading manager tests now lock the executed, fallback, rejection, failure, and missing-diary paths so attribution coverage stays at 100% in the exercised scenarios.【F:tests/orchestration/test_alpha_trade_runner.py†L485】【F:tests/orchestration/test_alpha_trade_runner.py†L1290】【F:tests/trading/test_trading_manager_execution.py†L1102】【F:tests/trading/test_trading_manager_execution.py†L1163】【F:tests/trading/test_trading_manager_execution.py†L1338】_
...
- [ ] **Timeâ€‘toâ€‘candidate** â‰¤ 24h (idea â†’ scored in replay). _Progress: Findings memory now exposes SLA analytics and a metrics CLI reports average/median/p90 turnaround plus breach details, returning a non-zero exit code whenever SLA breaches persist so pipeline monitors can fail fast, while the scheduler CLI quick-screens ideas, applies UCB-lite promotion with bounded `--max-quick`/`--max-full` budgets, and stamps explicit `evidence:quick-screen`/`evidence:full-eval` alongside `coverage:regression` notes so candidates enter replay within the 24 h envelope. Latest telemetry now backfills partially evaluated candidates by treating `progress_at` as the SLA fallback whenever `tested_at` is missing so breach analytics and CLI exit codes stay honest even while replay scoring is pending under regression coverage.【F:emp/core/findings_memory.py†L405-L439】【F:emp/cli/emp_cycle_metrics.py†L1-L70】【F:emp/cli/emp_cycle_scheduler.py†L32-L371】【F:tests/emp_cycle/test_time_to_candidate.py†L1-L121】【F:tests/emp_cycle/test_cycle_scheduler.py†L1-L86】_
- [x] **Promotion integrity**: 100% promoted strategies have ledger artifacts & pass regimeâ€‘grid gates. _Progress: Strategy registry now invokes the promotion guard before registering champions, surfaces guard failures as StrategyRegistryError, and regression coverage proves approvals succeed only after required regimes are recorded in the ledger and diary stores.【F:src/governance/strategy_registry.py†L348-L367】【F:tests/governance/test_strategy_registry.py†L271-L405】_
...
- [ ] **Attribution coverage** â‰¥ 90% (orders with belief + probes + brief explanation). _Progress: The loop runner now feeds the trimmed `_build_brief_explanation` output into trade metadata, intents, outcomes, and loop metadata regardless of diary availability, carrying policy/diary ids when present so attribution and coverage snapshots stay aligned.【F:src/orchestration/alpha_trade_runner.py†L889】【F:src/orchestration/alpha_trade_runner.py†L267】【F:src/orchestration/alpha_trade_runner.py†L278】【F:src/orchestration/alpha_trade_runner.py†L331】【F:src/orchestration/alpha_trade_runner.py†L343】【F:src/orchestration/alpha_trade_runner.py†L915】【F:src/orchestration/alpha_trade_runner.py†L923】【F:src/orchestration/alpha_trade_runner.py†L925】 Diaries from 2025-10-20 (e.g., the replay `diary-011424` and counterfactual mirror `counterfactual_diary-010924`) capture the updated attribution block with brief explanations, guardrail probes, and coverage snapshots.【F:artifacts/diaries/2025/10/20/diary-011424/diary.json†L474】【F:artifacts/diaries/2025/10/20/diary-011424/diary.json†L687】【F:artifacts/diaries/2025/10/20/counterfactual_diary-010924/counterfactual_diary.json†L412】【F:artifacts/diaries/2025/10/20/counterfactual_diary-010924/counterfactual_diary.json†L635】 TradingManager now prefers `brief_explanation`, rebuilds attribution from trade outcomes when intents lack it, and enforces coverage counters off the trimmed payloads to keep warning thresholds meaningful.【F:src/trading/trading_manager.py†L3947】【F:src/trading/trading_manager.py†L3953】【F:src/trading/trading_manager.py†L4160】【F:src/trading/trading_manager.py†L4166】【F:src/trading/trading_manager.py†L4180】 Regression coverage exercises execution success, failure, rejection, and missing-diary paths so attribution coverage remains intact across scenarios.【F:tests/orchestration/test_alpha_trade_runner.py†L485】【F:tests/orchestration/test_alpha_trade_runner.py†L1290】【F:tests/trading/test_trading_manager_execution.py†L1102】【F:tests/trading/test_trading_manager_execution.py†L1163】【F:tests/trading/test_trading_manager_execution.py†L1338】_
