...
from src.trading.mock_ctrader_interface import MarketData, Order, Position
...
class AdvancedRiskManager:
    ...
    def update_portfolio_state(
        ...
        self.portfolio_state.margin_level = (
            (equity / margin) if margin > 0 else float("inf")
        )
    ...
    def validate_signal(
        self, signal: StrategySignal, market_data: Dict[str, MarketData]
    ) -> Tuple[bool, str, Dict]:
        """
    ...
    def calculate_position_size(
        ...
        try:
            ...
            logger.debug(
                f"Calculated position size: {base_size:.4f} lots for {signal.symbol}"
            )
            return base_size
    ...
    def _validate_portfolio_limits(self, signal: StrategySignal) -> Tuple[bool, str]:
        ...
        if (
            ...
            <= -self.portfolio_state.total_equity * self.risk_limits.max_daily_loss
        ):
    ...
    def _validate_volatility_limits(
        self, signal: StrategySignal, market_data: Dict[str, MarketData]
    ) -> Tuple[bool, str]:
    ...
    def _calculate_risk_metadata(
        ...
        return {
            ...
            "strategy_performance": self._get_strategy_performance_metrics(
                signal.strategy_id
            ),
        }
    ...
    def _calculate_volatility_adjustment(
        self, symbol: str, market_data: Dict[str, MarketData]
    ) -> float:
        ...
        if volatility > 0:
            adjustment = min(
                2.0, max(0.5, 0.15 / volatility)
            )  # Cap between 0.5x and 2x
            return adjustment
    ...
    def _calculate_exposure_metrics(self, positions: List[Position]):
        ...
        self.risk_metrics.total_exposure = (
            total_exposure / self.portfolio_state.total_equity
            if self.portfolio_state.total_equity > 0
            else 0
        )

        self.risk_metrics.leverage_ratio = (
            self.portfolio_state.total_margin / self.portfolio_state.total_equity
            if self.portfolio_state.total_equity > 0
            else 0
        )
        ...
        if len(returns) > 0:
            ...
            if len(downside_returns) > 0:
                ...
                self.risk_metrics.sortino_ratio = (
                    (np.mean(returns) * 252) / downside_deviation
    ...
    def _calculate_correlation_metrics(
        ...
        for pos in positions:
            if pos.symbol_id in market_data:
                ...
                current_price = (
                    market_data[pos.symbol_id].bid
                    if pos.side.value == "buy"
                    else market_data[pos.symbol_id].ask
                )
        ...
        if len(position_returns) >= 2:
            ...
            avg_correlation = (np.sum(correlation_matrix) - len(correlation_matrix)) / (
    ...
    def _calculate_symbol_volatility(
        self, symbol: str, market_data: Dict[str, MarketData]
    ) -> float:
        """Calculate symbol volatility."""
        # This would use historical data in a real implementation
    ...
    def _generate_risk_alerts(self) -> List[str]:
        ...
        if self.risk_metrics.total_exposure > self.risk_limits.max_total_exposure * 0.8:
            alerts.append(
                f"High exposure warning: {self.risk_metrics.total_exposure:.2%}"
            )

        if self.risk_metrics.leverage_ratio > self.risk_limits.max_leverage * 0.8:
            alerts.append(
                f"High leverage warning: {self.risk_metrics.leverage_ratio:.2f}"
            )

        if self.risk_metrics.correlation_score > self.risk_limits.max_correlation * 0.8:
            alerts.append(
                f"High correlation warning: {self.risk_metrics.correlation_score:.2f}"
            )
        ...
        if (
            self.portfolio_state.daily_pnl
            < -self.portfolio_state.total_equity * self.risk_limits.max_daily_loss * 0.8
        ):
...
def main():
    ...
    parser.add_argument(
        "--test-validation", action="store_true", help="Test signal validation"
    )
    parser.add_argument(
        "--test-position-sizing", action="store_true", help="Test position sizing"
    )
    parser.add_argument(
        "--test-risk-metrics", action="store_true", help="Test risk metrics calculation"
    )
    ...
    if args.test_risk_metrics:
        ...
        from src.trading.mock_ctrader_interface import OrderSide, Position
