...
class ChaosAdaptationSystem:
    ...
    async def analyze_chaos_adaptation(self, market_data: pd.DataFrame) -> ChaosAdaptation:
        ...
        try:
            ...
            volatility_harvesting = await self.volatility_harvester.identify_opportunities(
                market_data
            )
    ...
    def _calculate_chaos_confidence(
        self,
        black_swan: BlackSwanDetection,
        volatility_harvesting: VolatilityHarvesting,
        crisis_alpha: CrisisAlpha,
        ...
        return float(min(max(float(np.mean(confidences)), 0.0), 1.0))

    async def _generate_antifragile_strategies(
        self,
        black_swan: BlackSwanDetection,
        volatility_harvesting: VolatilityHarvesting,
        crisis_alpha: CrisisAlpha,
        ...
        if black_swan.detected:
            strategies.extend(
                ["increase_cash_position", "reduce_leverage", "implement_tail_risk_hedging"]
            )

        if volatility_harvesting.opportunity_score > 0.5:
            strategies.extend(
                ["volatility_selling_strategies", "gamma_scalping", "variance_swap_positions"]
            )

        if crisis_alpha.alpha_potential > 0.3:
            strategies.extend(
                ["crisis_alpha_strategies", "momentum_reversal_trades", "liquidity_premium_capture"]
            )
...
class BlackSwanDetector:
    ...
    def _calculate_price_change(self, data: pd.DataFrame) -> float:
        ...
        return float((data["close"].iloc[-1] - data["close"].iloc[0]) / data["close"].iloc[0])
    ...
    async def identify_opportunities(self, data: pd.DataFrame) -> VolatilityHarvesting:
        ...
        try:
            ...
            return VolatilityHarvesting(
                ...
                optimal_strategies=(
                    ["volatility_selling", "gamma_scalping"]
                    if opportunity_score > 0.5
                    else ["basic_strategies"]
                ),
        ...
        try:
            ...
            return CrisisAlpha(
                ...
                strategy_recommendations=(
                    ["momentum_reversal", "liquidity_premium"]
                    if alpha_potential > 0.3
                    else ["hold"]
                ),
