...
4. **Regime Classification** – `RegimeFSM` maps posterior strength/confidence into qualitative regimes (calm/balanced/bullish/bearish), emits structured transition events with latency/volatility metadata, and keeps a bounded transition history plus lineage attached for diagnostics.【F:src/understanding/belief.py†L695-L878】【F:tests/understanding/test_belief_updates.py†L327-L394】
...
- **Configuration** – Runtime extras on `SystemConfig` can tighten sparsity or force excitatory-only behaviour. Construct the router via `UnderstandingRouter.from_system_config(config)` and set keys such as `FAST_WEIGHT_MAX_ACTIVE_FRACTION` or `FAST_WEIGHT_EXCITATORY_ONLY` to tune thresholds without modifying code.【F:src/understanding/router.py†L205-L223】【F:docs/context/examples/understanding_router.md†L73-L105】
