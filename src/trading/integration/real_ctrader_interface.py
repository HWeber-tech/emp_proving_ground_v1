...
"
    ...
    async def exchange_code_for_token(self, authorization_code: str, redirect_uri: str) -> Dict[str, str]:
...
class RealCTraderInterface:
    ...
    async def _get_trading_accounts(self):
        ...
        headers = {'Authorization': f'Bearer {await self.token_manager.get_access_token()}'}
        ...
        async with self.session.get(url, headers=headers) as response:
            if response.status == 200:
                ...
                if accounts:
                    ...
                    self.config.account_id = str(self.account_info['ctidTraderAccountId'])
    ...
    async def _handle_order_update(self, message: Dict[str, Any]):
        ...
        try:
            order = Order(
                ...
                stop_loss=float(message.get('stopLoss', 0)) if message.get('stopLoss') else None,
                take_profit=float(message.get('takeProfit', 0)) if message.get('takeProfit') else None,
                status=OrderStatus(message['status']),
                filled_volume=float(message.get('filledVolume', 0)),
                average_price=float(message.get('averagePrice', 0)) if message.get('averagePrice') else None,
                timestamp=datetime.fromisoformat(message['timestamp'])
            )
    ...
    async def place_order(self, symbol_name: str, order_type: OrderType, side: OrderSide,
                         volume: float, price: Optional[float] = None, stop_loss: Optional[float] = None,
        ...
        try:
            ...
            async with self.session.put(url, headers=headers, json=data) as response:
                if response.status == 200:
                    ...
                    logger.error(f"Failed to modify position {position_id}: {response.status}")
                    return False
        ...
        try:
            ...
            async with self.session.delete(url, headers=headers) as response:
                if response.status == 200:
                    ...
                    logger.error(f"Failed to close position {position_id}: {response.status}")
                    return False
