...
- Implement deterministic risk API enforcement with telemetry topics (policy,
  ...
  - Progress: Throughput monitoring pairs with a new `TradingManager.assess_throughput_health()` helper so ops checks can assert latency and ingest lag budgets from a single snapshot during high-frequency drills, with regression coverage ensuring healthy verdicts under throttle-heavy replays.【F:src/trading/trading_manager.py†L1201-L1353】【F:docs/performance/trading_throughput_monitoring.md†L1-L84】【F:tests/trading/test_trading_manager_execution.py†L1799-L1905】
