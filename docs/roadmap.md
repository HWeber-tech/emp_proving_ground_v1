...
- [ ] `[core]` Live market data ingest configured (API keys, symbols, session calendars). _Progress: Institutional ingest builder now caches ReferenceDataLoader instrument inventory alongside API key posture, FX session calendars, macro calendars, and symbol metadata (contract size, pip decimals, margin currency, swap timing, long/short swap rates) for the configured symbol list, short-circuits to an empty inventory when no symbols are configured so unprovisioned environments never leak the full catalog, and still constrains the exported inventory to only configured symbols so manifests omit unsubscribed pairs, while managed ingest connectors CLI surfaces both inventory and metadata snapshots with swap rate context so operators receive margin-aware manifests; regression coverage now locks EURUSD long/short swap rates across the ingest config and managed connectors CLI while live wiring remains outstanding.【F:src/data_foundation/ingest/configuration.py†L199-L220】【F:src/data_foundation/ingest/configuration.py†L955-L976】【F:tests/data_foundation/test_timescale_config.py†L320-L340】【F:tools/operations/managed_ingest_connectors.py†L420-L473】【F:tests/tools/test_managed_ingest_connectors.py†L60-L80】_
