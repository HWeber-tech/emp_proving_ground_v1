...
class _StubPredictiveModeler:
    ...
    async def predict_market_scenarios(self, *_, **__) -> list[SimpleNamespace]:
        return [
...
async def test_phase3_orchestrator_pipeline_smoke(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    _install_module(
        ...
        {"MarketGAN": _StubMarketGAN},
    )
    _install_module(
        monkeypatch,
        "src.thinking.adversarial.red_team_ai",
        {"RedTeamAI": _StubRedTeamAI},
    )
    _install_module(
        monkeypatch,
        "src.thinking.competitive.competitive_intelligence_system",
        {"CompetitiveIntelligenceSystem": _StubCompetitiveIntelligence},
    )
    _install_module(
        monkeypatch,
        "src.thinking.ecosystem.specialized_predator_evolution",
        {"SpecializedPredatorEvolution": _StubSpecializedEvolution},
    )
    _install_module(
        monkeypatch,
        "src.thinking.prediction.predictive_market_modeler",
        {"PredictiveMarketModeler": _StubPredictiveModeler},
    )
