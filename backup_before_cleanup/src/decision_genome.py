...
class DecisionGenome:
        ...
        for i, (timestamp, row) in enumerate(market_data.iterrows()):
            ...
            if position != 0:
                if position > 0:
                    ...
                    current_equity += float((entry_price - row['close']) * abs(position))
    ...
    def _evaluate_decision_tree(self, sensory_reading) -> str:
        ...
        try:
            ...
            if hasattr(sensory_reading, 'macro_trend') and hasattr(sensory_reading, 'macro_strength'):
            ...
            if hasattr(sensory_reading, 'institutional_flow') and hasattr(sensory_reading, 'institutional_confidence'):
                if sensory_reading.institutional_flow > 0:
                    buy_score += institutional_weight * sensory_reading.institutional_flow * sensory_reading.institutional_confidence
                else:
                    sell_score += institutional_weight * abs(sensory_reading.institutional_flow) * sensory_reading.institutional_confidence
            
            # Momentum contribution
            if hasattr(sensory_reading, 'momentum_score') and hasattr(sensory_reading, 'overall_sentiment'):
                if sensory_reading.momentum_score > 0:
                    buy_score += momentum_weight * sensory_reading.momentum_score * sensory_reading.overall_sentiment
                else:
                    sell_score += momentum_weight * abs(sensory_reading.momentum_score) * (1 - sensory_reading.overall_sentiment)
                ...
                try:
                    ...
                    if 'close' in row:
                        ...
                        if hasattr(self, '_prev_close'):
                            ...
                            # Simple momentum
                            reading.momentum_score = change * 1000  # Scale for reasonable range
                            
                            # Simple sentiment (based on volume and price movement)
                            if 'volume' in row:
                                volume_factor = min(1.0, row['volume'] / 5000)
                                reading.overall_sentiment = 0.5 + (reading.momentum_score * 0.3) * volume_factor
                            else:
                                reading.overall_sentiment = 0.5 + (reading.momentum_score * 0.3)
...
class FitnessEvaluator:
    ...
    def __init__(self, data_storage: TickDataStorage, instrument: str = "EURUSD"):
        ...
        logger.info(f"FitnessEvaluator initialized with anti-fragility metrics for {instrument}")
    ...
    def evaluate_genome(self, genome: DecisionGenome, sensory_cortex) -> Dict[str, float]:
        ...
        try:
            ...
            if not regime_results:
                logger.warning("No regime data available for evaluation - using synthetic data")
        ...
        try:
            ...
            if trending_data is not None and not trending_data.empty:
                ...
                if all(col in trending_data.columns for col in required_columns):
                    ...
                    logger.warning(f"TRENDING data missing required columns. Available: {list(trending_data.columns)}")
            ...
            if ranging_data is not None and not ranging_data.empty:
                if all(col in ranging_data.columns for col in required_columns):
                    ...
                    logger.warning(f"RANGING data missing required columns. Available: {list(ranging_data.columns)}")
            ...
            if volatile_data is not None and not volatile_data.empty:
                if all(col in volatile_data.columns for col in required_columns):
                    ...
                    logger.warning(f"VOLATILE data missing required columns. Available: {list(volatile_data.columns)}")
                    self.regime_datasets['VOLATILE'] = None
            else:
                self.regime_datasets['VOLATILE'] = None
            
            loaded_count = sum(1 for dataset in self.regime_datasets.values() if dataset is not None)
    ...
    def _create_synthetic_market_data(self) -> Optional[pd.DataFrame]:
        ...
        try:
            ...
            for i in range(999):
                ...
                change = np.random.normal(0.0001, 0.001)  # Small positive drift, realistic volatility
    ...
    def _calculate_efficiency_score(self, regime_results: Dict[str, Dict]) -> float:
        ...
        sharpe_score = max(0.0, min(1.0, float(avg_sharpe) / 2.0))  # Cap at 2.0 Sharpe
        sortino_score = max(0.0, min(1.0, float(avg_sortino) / 2.0))  # Cap at 2.0 Sortino
        profit_factor_score = max(0.0, min(1.0, float(avg_profit_factor - 1.0) / 2.0))  # Cap at 3.0 PF
