...
- Finish the task supervision rollout across runtime and operational helpers.【F:docs/technical_debt_assessment.md†L33-L56】
  ...
  - Progress: Final dry run harness now delegates pumps, timeout, and wait tasks to a shared `TaskSupervisor`, cancelling owned workloads on exit with regression coverage proving supervised task registration and cleanup so UAT rehearsals inherit the same guardrails as runtime entrypoints.【F:src/operations/final_dry_run.py†L203-L268】【F:tests/operations/test_final_dry_run.py†L138-L171】
