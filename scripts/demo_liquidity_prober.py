...
async def demonstrate_liquidity_probing():
    ...
    await broker.start()
    liquidity_prober = LiquidityProber(
        broker, config={"probe_size": 0.001, "timeout_seconds": 2.0, "max_concurrent_probes": 5}
    )
...
async def demonstrate_risk_gateway_integration():
    ...
    await broker.start()
    liquidity_prober = LiquidityProber(
        broker, config={"probe_size": 0.001, "timeout_seconds": 2.0, "max_concurrent_probes": 3}
    )
...
async def demonstrate_iceberg_detection():
    ...
    liquidity_prober = LiquidityProber(
        iceberg_broker,
        config={"probe_size": 0.001, "timeout_seconds": 2.0, "max_concurrent_probes": 5},
    )
