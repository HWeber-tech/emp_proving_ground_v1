...
class PerformanceTracker:
    ...
    def update_position(self, position_data: Dict) -> None:
        ...
        if 'pnl' in position_data:
            self.current_balance += position_data['pnl']
    ...
    def record_trade(self, trade_data: Dict) -> None:
        ...
        if 'entry_price' in trade_data and 'exit_price' in trade_data:
            trade_data['pnl'] = (trade_data['exit_price'] - trade_data['entry_price']) * trade_data['size']
            trade_data['return'] = trade_data['pnl'] / (trade_data['entry_price'] * trade_data['size'])
        ...
        self.strategy_performance[strategy]['total_pnl'] += trade_data.get('pnl', 0)
    ...
    def update_regime_performance(self, regime: str, performance: float) -> None:
        ...
        self.regime_performance[regime]['total_return'] += performance
        self.regime_performance[regime]['avg_return'] = (
            self.regime_performance[regime]['total_return'] / 
    ...
    def calculate_metrics(self, force_recalculate: bool = False) -> PerformanceMetrics:
        ...
        if (self.metrics is not None and 
            self.last_calculation is not None and 
            datetime.now() - self.last_calculation < timedelta(minutes=5) and
        ...
        logger.info(f"Performance metrics calculated: {total_return:.2%} total return, {sharpe_ratio:.2f} Sharpe")
        return self.metrics
    ...
    def _calculate_strategy_performance(self) -> Dict[str, Dict[str, Any]]:
        ...
        for strategy, data in self.strategy_performance.items():
            if data['trades'] > 0:
                ...
                avg_return = data['total_return'] / data['trades']
    ...
    def _calculate_correlation_matrix(self) -> pd.DataFrame:
        ...
        for strategy, data in self.strategy_performance.items():
            if data['trades'] > 0:
                ...
                strategy_returns[strategy] = data['total_return'] / data['trades']
