...
- Progress: An `OperationalBackboneService` facade now coordinates the managed Timescale/Redis/Kafka pipeline, launching ingest slices, Kafka streaming, and the scheduler under a shared lifecycle while exposing cache metrics, connectivity reports, and TaskSupervisor snapshots; async guardrails cover ingest cycles, streaming fan-out, and scheduler start/stop flows.【F:src/data_foundation/pipelines/backbone_service.py†L1-L219】【F:tests/data_foundation/test_operational_backbone_service.py†L1-L200】
