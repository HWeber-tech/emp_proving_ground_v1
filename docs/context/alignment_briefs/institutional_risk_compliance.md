...
- Progress: Execution pipeline now enforces configurable trade throttles via the trading manager, producing throttle posture snapshots and regression coverage that proves bursty strategies are deferred according to governance policy, with dedicated unit tests validating window, cooldown, retry metadata, and the burst-aware messaging surfaced to diaries and dashboards.【F:src/trading/execution/trade_throttle.py†L15-L205】【F:src/trading/trading_manager.py†L338-L420】【F:tests/trading/test_trading_manager_execution.py†L1641-L1721】
