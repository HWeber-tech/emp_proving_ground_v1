...
- Progress: FAISS pattern memory now captures timestamped backups, prunes old
  snapshots, and restores metadata/index state so adaptive loops can rehearse
  recovery drills with deterministic tests guarding the workflow.【F:src/sentient/memory/faiss_pattern_memory.py†L53-L324】【F:tests/sentient/test_memory_backup.py†L28-L70】
