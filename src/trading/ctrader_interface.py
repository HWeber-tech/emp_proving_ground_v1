...
class CTraderInterface:
    ...
    async def connect(self) -> bool:
        ...
        try:
            ...
            # Create client
            self.client = Client(
                self.config.host, self.config.port, on_message=self._on_message_received
            )
    ...
    async def place_order(
        ...
        try:
            ...
            if order_type == OrderType.MARKET:
                ...
                if stop_loss:
                    order_req.stopLoss = int(
                        stop_loss * (10 ** self.symbols_cache[symbol_id]["digits"])
                    )
                if take_profit:
                    order_req.takeProfit = int(
                        take_profit * (10 ** self.symbols_cache[symbol_id]["digits"])
                    )
                ...
                order_req.volume = volume_int
                order_req.price = int(
                    price * (10 ** self.symbols_cache[symbol_id]["digits"])
                )
    ...
    def _handle_error_event(self, message: Protobuf):
        ...
        try:
            ...
            error_msg = (
                f"cTrader Error: {error_res.description} (Code: {error_res.errorCode})"
            )
    ...
    def _get_proto_trade_side(self, side: OrderSide) -> int:
        ...
        mapping = {OrderSide.BUY: 1, OrderSide.SELL: 2}  # BUY  # SELL
...
class TokenManager:
    ...
    def exchange_code_for_token(
        self, auth_code: str, redirect_uri: str = "http://localhost/"
    ) -> Optional[Dict]:
    ...
    def get_trading_accounts(self, access_token: str) -> Optional[List[Dict]]:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                logger.error(
                    f"Failed to get trading accounts: {response.status_code} - {response.text}"
                )
                return None
...
def main():
    ...
    def on_price_update(market_data):
        print(
            f"Price Update: {market_data.symbol_name} Bid={market_data.bid}, Ask={market_data.ask}"
        )
