...
class OrderbookOrgan(SensoryOrgan):
    """Sensory organ for processing order book data."""
    ...
    def _calculate_spread_signal(self, current_spread: float) -> OrderbookSignal:
        ...
        confidence = min(abs(spread_deviation) * 2, 1.0)
    ...
    def _calculate_pressure_signal(self, data: MarketData) -> OrderbookSignal:
        ...
        normalized_pressure = np.tanh(pressure_signal * 1000)  # Scale for forex
        
        # Calculate confidence based on trend strength
        confidence = min(abs(pressure_signal) * 500, 1.0)
    ...
    def _calculate_depth_signal(self, data: MarketData) -> OrderbookSignal:
        ...
        depth_signal = max(0, 1.0 - spread_volatility / 2.0)  # Normalize
    ...
    def _calculate_flow_signal(self, data: MarketData) -> OrderbookSignal:
        ...
        spread = data.ask - data.bid
        confidence = min(1.0, 1.0 / (spread * 10000))  # Smaller spread = higher confidence
