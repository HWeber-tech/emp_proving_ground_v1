...
- Progress: Fusion engine and demos now expose an `analyze_market_understanding` coroutine (with a deprecated alias for legacy callers) so orchestration helpers, scripts, and docs share the understanding-first API while maintaining compatibility under regression coverage.【F:src/orchestration/enhanced_understanding_engine.py†L203-L299】【F:scripts/minimal_sensory_demo.py†L117-L188】【F:src/sensory/README.md†L153-L161】
