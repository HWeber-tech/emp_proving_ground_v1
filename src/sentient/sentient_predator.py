...
class SentientPredator:
    ...
    async def process_closed_trade(self, trade_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        memory_id = self.memory.add_experience(vector, {
            'learning_signal': {
    ...
    def _create_vector_representation(self, signal: LearningSignal) -> np.ndarray:
        ...
        vector = np.array([
            ...
            features.get('liquidity_ratio', 1),
            features.get('order_flow_imbalance', 0),
            features.get('microstructure_score', 0),
            context.get('volatility', 0),
            context.get('spread', 0),
            signal.pnl,
            signal.duration / 3600  # Normalize duration to hours
        ])
    ...
    async def _apply_adaptations(self, adaptations: List[TacticalAdaptation]) -> List[TacticalAdaptation]:
