...
class MockBroker:
    ...
    async def place_order(self, symbol_name: str, order_type: str, side: str, 
        ...
        logger.info(f"Mock order: {side} {volume} {symbol_name} @ {price} -> filled {filled_volume}")
        return order_id
...
class LiquidityProber:
    ...
    async def probe_liquidity(self, symbol: str, price_levels: List[float], side: str) -> Dict[float, float]:
    ...
    def calculate_liquidity_confidence_score(self, probe_results: Dict[float, float], intended_volume: float) -> float:
        ...
        logger.info(f"Liquidity confidence score: {score:.3f} (coverage: {coverage_ratio:.3f}, distribution: {distribution_quality:.3f})")
        return score
...
class RiskGateway:
    ...
    async def validate_trade_intent(self, intent: TradeIntent, portfolio_state: Dict[str, Any]) -> Optional[TradeIntent]:
        """Validate trade intent with liquidity checking"""
        quantity = float(intent.quantity)
        
        if quantity < self.probe_threshold:
            logger.info(f"Trade below threshold ({quantity} < {self.probe_threshold}), skipping liquidity probe")
            return intent
        ...
        if confidence_score < 0.3:
            logger.warning(f"Trade REJECTED: insufficient liquidity (score: {confidence_score:.3f})")
            return None
...
async def demonstrate_iceberg_detection():
    ...
    class IcebergBroker(MockBroker):
        ...
        async def place_order(self, symbol_name, order_type, side, volume, price=None):
            ...
            logger.info(f"Iceberg order: {side} {volume} @ {price} -> {filled_volume} (fill rate: {fill_rate})")
            return order_id
