...
class TacticalAdaptationEngine:
    ...
    async def generate_adaptations(
        ...
        try:
            ...
            similar_experiences = await cast(Any, self.pattern_memory).find_similar_experiences(
            ...
            pattern_analysis = await self._analyze_patterns(similar_experiences, learning_signal)

            # Generate adaptations based on analysis
            adaptations.extend(
                await self._generate_parameter_adaptations(pattern_analysis, current_strategy_state)
            )

            adaptations.extend(
                await self._generate_risk_adaptations(pattern_analysis, current_strategy_state)
            )
    ...
    async def _analyze_patterns(
        self, similar_experiences: List[tuple[float, Any]], learning_signal: LearningSignal
    ) -> dict[str, object]:
        """Analyze patterns from similar experiences."""
        try:
            ...
            weighted_avg = (
                sum(weighted_outcomes) / len(weighted_outcomes) if weighted_outcomes else 0
            )
    ...
    async def _generate_parameter_adaptations(
        self, pattern_analysis: dict[str, object], current_strategy_state: dict[str, object]
    ) -> List[TacticalAdaptation]:
        ...
        try:
            ...
            avg_outcome_f = float(cast(float, avg_outcome))
            current_outcome_f = float(cast(float, current_outcome))
            if avg_outcome_f > 0 and current_outcome_f < 0:
            ...
            elif avg_outcome_f < 0 and current_outcome_f > 0:
    ...
    async def _generate_risk_adaptations(
        ...
        try:
            ...
            win_rate_f = float(cast(float, win_rate))
            if win_rate_f < 0.4:
                ...
                adaptations.append(
                    TacticalAdaptation(
                        ...
                        duration_ticks=200,
                        confidence=Decimal(str(1 - win_rate_f)),
                ...
                adaptations.append(
                    TacticalAdaptation(
                        ...
                        duration_ticks=200,
                        confidence=Decimal(str(win_rate_f)),
                        metadata={"risk_based": True},
                    )
                )
    ...
    async def _store_adaptation(self, adaptation: TacticalAdaptation) -> None:
        ...
        try:
            ...
            await self.state_store.set(key, adaptation.json(), expire=86400)  # 1 day
        except Exception as e:
            logger.error(f"Error storing adaptation: {e}")

    async def apply_adaptations(
        self, adaptations: List[TacticalAdaptation], strategy_id: str
    ) -> bool:
        """
