...
- Add regression tickets/tests for uncovered modules (`operational.metrics`,
  ...
  - Progress: Kibana dashboard deployment CLI zips Saved Objects exports, supports API key or basic auth flows, and surfaces Saved Objects errors so operations can refresh observability panels without manual cURL scripts, with pytest coverage validating both success and failure paths.【F:tools/observability/deploy_kibana_dashboards.py†L1-L190】【F:tools/observability/deploy_kibana_dashboards.py†L200-L270】【F:tests/tools/test_deploy_kibana_dashboards.py†L1-L78】
