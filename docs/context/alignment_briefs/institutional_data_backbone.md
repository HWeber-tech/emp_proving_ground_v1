...
- Progress: Timescale connection settings accept pooling overrides (`TIMESCALEDB_POOL_SIZE`, `TIMESCALEDB_MAX_OVERFLOW`, `TIMESCALEDB_POOL_TIMEOUT`, `TIMESCALEDB_POOL_RECYCLE`, `TIMESCALEDB_POOL_PRE_PING`) and apply them only for PostgreSQL/Timescale targets while leaving SQLite fallbacks untouched; regression coverage verifies Postgres vs SQLite behaviour so institutional deployments can tune pool sizing directly from `SystemConfig` extras.【F:src/data_foundation/persist/timescale.py†L111-L255】【F:tests/data_foundation/test_timescale_connection_settings.py†L18-L90】
