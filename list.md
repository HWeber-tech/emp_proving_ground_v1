...
26 - Audit orchestration imports in [src/orchestration/enhanced_intelligence_engine.py](src/orchestration/enhanced_intelligence_engine.py:1) and ensure they import canonical `src.sensory.enhanced.*` modules (no legacy shims)
27 - Confirm no residual references to legacy `market_intelligence.dimensions.*`; callers must import from `src.sensory.enhanced.*`
...
43 - Provide stubs for sensory enhanced dimension aggregators in `src/sensory/enhanced/` where dynamic exports remain
