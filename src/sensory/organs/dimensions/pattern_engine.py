...
class FractalDetector:
        ...
        for move in significant_moves:
            ...
            if extension_levels:
                pattern = FractalPattern(
                    ...
                    start_time=cast(datetime, move["start_time"]),
                    end_time=cast(datetime, move["end_time"]),
    ...
    def _identify_significant_swings(self, data: pd.DataFrame) -> list[dict[str, Any]]:
        """Identify significant price swings (delegated to features module)."""
        return cast(list[dict[str, Any]], identify_significant_swings(data))

    def _calculate_fibonacci_levels(self, swing: dict[str, Any]) -> list[float]:
        """Calculate Fibonacci retracement levels (delegated to features module)."""
        return cast(list[float], calculate_fibonacci_levels(swing))

    def _identify_significant_moves(self, data: pd.DataFrame) -> list[dict[str, Any]]:
        """Identify significant price moves for extension patterns (delegated)."""
        return cast(list[dict[str, Any]], identify_significant_moves(data))

    def _calculate_extension_levels(self, move: dict[str, Any]) -> list[float]:
        """Calculate Fibonacci extension levels (delegated to features module)."""
        return cast(list[float], calculate_extension_levels(move))

    def _validate_elliott_sequence(
        self, data: pd.DataFrame, highs: list[int], lows: list[int]
...
class HarmonicAnalyzer:
    ...
    def _find_xabcd_patterns(self, data: pd.DataFrame) -> List[Dict[str, Any]]:
        """Find potential XABCD harmonic patterns"""
        patterns = []

        # Find significant turning points
        highs = find_peaks(
            np.asarray(data["high"].values, dtype=float), distance=10, prominence=0.02
        )[0]
        lows = find_peaks(
            -np.asarray(data["low"].values, dtype=float), distance=10, prominence=0.02
        )[0]
        ...
        for idx in highs:
            points.append(
                {"time": data.index[idx], "price": data["high"].iloc[idx], "type": "high"}
            )
        for idx in lows:
    ...
    def _validate_gartley_ratios(self, pattern: Dict[str, Any]) -> bool:
        """Validate Gartley pattern ratios"""
        ratios = pattern["ratios"]

        # Gartley ratios: AB=0.618XA, BC=0.382-0.886AB, CD=1.27-1.618BC, AD=0.786XA
        return bool(
            0.5 <= ratios["ab_xa"] <= 0.7
            and 0.3 <= ratios["bc_ab"] <= 0.9
            and 1.2 <= ratios["cd_bc"] <= 1.7
            and 0.7 <= ratios["ad_xa"] <= 0.85
        )

    def _validate_butterfly_ratios(self, pattern: Dict[str, Any]) -> bool:
        """Validate Butterfly pattern ratios"""
        ratios = pattern["ratios"]

        # Butterfly ratios: AB=0.786XA, BC=0.382-0.886AB, CD=1.618-2.618BC, AD=1.27XA
        return bool(
            0.7 <= ratios["ab_xa"] <= 0.9
            and 0.3 <= ratios["bc_ab"] <= 0.9
            and 1.5 <= ratios["cd_bc"] <= 2.7
            and 1.2 <= ratios["ad_xa"] <= 1.4
        )

    def _validate_crab_ratios(self, pattern: Dict[str, Any]) -> bool:
        """Validate Crab pattern ratios"""
        ratios = pattern["ratios"]

        # Crab ratios: AB=0.382-0.618XA, BC=0.382-0.886AB, CD=2.24-3.618BC, AD=1.618XA
        return bool(
            0.3 <= ratios["ab_xa"] <= 0.7
            and 0.3 <= ratios["bc_ab"] <= 0.9
            and 2.0 <= ratios["cd_bc"] <= 3.7
            and 1.5 <= ratios["ad_xa"] <= 1.7
        )

    def _validate_bat_ratios(self, pattern: Dict[str, Any]) -> bool:
        """Validate Bat pattern ratios"""
        ratios = pattern["ratios"]

        # Bat ratios: AB=0.382-0.5XA, BC=0.382-0.886AB, CD=1.618-2.618BC, AD=0.886XA
        return bool(
            0.3 <= ratios["ab_xa"] <= 0.6
            and 0.3 <= ratios["bc_ab"] <= 0.9
            and 1.5 <= ratios["cd_bc"] <= 2.7
            and 0.8 <= ratios["ad_xa"] <= 0.95
        )
...
class PatternValidator:
    ...
    async def validate_patterns(
        self, fractals: List[FractalPattern], harmonics: List[HarmonicPattern], data: pd.DataFrame
    ) -> float:
        ...
        try:
            ...
            harmonic_strength = (
                float(np.mean([h.confidence for h in harmonics])) if harmonics else 0.0
            )
            ...
            total_strength: float = (
                fractal_strength * 0.4 + harmonic_strength * 0.4 + data_quality * 0.2
            )
