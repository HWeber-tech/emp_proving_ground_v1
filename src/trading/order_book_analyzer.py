...
class OrderBookAnalyzer:
    ...
    def _analyze_microstructure(
        ...
        spread_bps = (
            (spread / snapshot.mid_price) * 10000 if snapshot.mid_price > 0 else 0.0
        )
        ...
        is_imbalanced = abs(liquidity_imbalance) > 0.3
    ...
    def _calculate_order_flow_imbalance(self, symbol: str) -> float:
        ...
        # Last 5 snapshots
        recent = list(self.microstructure_history[symbol])[-5:]
        if not recent:
            return 0.0
    ...
    def _calculate_buy_pressure(self, symbol: str) -> float:
        ...
        # Last 3 snapshots
        recent = list(self.microstructure_history[symbol])[-3:]
        if not recent:
            return 0.0
    ...
    def _calculate_market_impact(
        self, snapshot: OrderBookSnapshot, volume: float
    ) -> float:
    ...
    def _calculate_price_volatility(self, symbol: str) -> float:
        ...
        # Last 10 snapshots
        recent = list(self.order_book_history[symbol])[-10:]
        if len(recent) < 2:
            return 0.0
    ...
    def _calculate_volume_volatility(self, symbol: str) -> float:
        ...
        # Last 10 snapshots
        recent = list(self.microstructure_history[symbol])[-10:]
        if len(recent) < 2:
            return 0.0
    ...
    def get_market_analysis(self, symbol: str) -> Dict[str, Any]:
        ...
        liquidity_trend = self._calculate_trend(
            [m.total_liquidity for m in recent_history]
        )
        imbalance_trend = self._calculate_trend(
            [m.liquidity_imbalance for m in recent_history]
        )
    ...
    def _generate_trading_signals(
        ...
        positive_signals = sum(
            1 for signal in signals.values() if signal in ["positive", "buy"]
        )
        negative_signals = sum(
            1 for signal in signals.values() if signal in ["negative", "sell"]
        )
    ...
    def get_liquidity_analysis(self, symbol: str, volume: float) -> Dict[str, Any]:
        ...
        bid_execution_prob = (
            min(1.0, available_bid_liquidity / volume) if volume > 0 else 0.0
        )
        ask_execution_prob = (
            min(1.0, available_ask_liquidity / volume) if volume > 0 else 0.0
        )
