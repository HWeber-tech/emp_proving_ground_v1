...
- Progress: The executable anomaly organ now tracks the last frame timestamp, length, and close value so it can ignore replayed candles while still ingesting new ticks, keeping rolling windows stable during ingest replays under regression coverage while a fresh regression asserts high-magnitude spikes escalate to alert/critical posture with z-score justification and sensory provenance.【F:src/sensory/organs/dimensions/executable_organs.py†L376-L480】【F:tests/sensory/test_dimension_organs.py†L240-L314】
...
- Progress: Namespace integrity guardrail now scans `src/`, `tools/`, `scripts/`, `examples/`, and `emp/` for banned legacy prefixes—including the retired macro-intelligence shim and accuracy validator—so regressions cannot reintroduce deprecated sensory or intelligence modules, and the demo/replay scripts import the canonical sensory namespaces under regression coverage.【F:tests/current/test_namespace_integrity.py†L11-L87】【F:scripts/sensory_demo.py†L1-L160】【F:scripts/replay_into_sensory.py†L1-L93】【F:scripts/minimal_sensory_demo.py†L1-L160】
...
- Progress: Guardrail coverage now replays a volatility spike through `RealSensoryOrgan`, the HOW engine, and the belief emitter, asserting drift summaries, anomaly payloads, and belief posterior features all materialise so the sensory-to-understanding rehearsal path stays intact on canonical surfaces.【F:tests/understanding/test_sensory_belief_pipeline.py†L84-L139】【F:src/sensory/real_sensory_organ.py†L190-L235】【F:src/understanding/belief.py†L649-L706】 Latest revision backfills `quality`/`lineage` metadata for bare sensors, synthesising canonical timestamps, strengths, and provenance records so fused snapshots remain auditable even when upstream organs omit metadata, with targeted guardrails proving each dimension inherits the generated payloads.【F:src/sensory/real_sensory_organ.py†L184-L278】【F:tests/sensory/test_real_sensory_organ_metadata.py†L1-L71】
...
- Progress: Belief buffer now harvests fused-sensor `quality` envelopes, emitting quality confidence, strength, and data-quality features for each dimension; the golden snapshot and sensory pipeline guardrails verify the enriched posterior vector persists through volatility spikes and Hebbian update sweeps.【F:src/understanding/belief.py†L191-L214】【F:tests/understanding/golden/belief_snapshot.json†L1-L120】【F:tests/understanding/test_belief_updates.py†L184-L276】【F:tests/understanding/test_sensory_belief_pipeline.py†L136-L140】
