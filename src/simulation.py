...
try:
    from src.sensory.core.base import (InstrumentMeta, MarketData,
                                       OrderBookSnapshot)
    from src.sensory.orchestration.master_orchestrator import (
        MasterOrchestrator, SynthesisResult)
...
class MarketSimulator:
    ...
    def __init__(
        self, data_storage, initial_balance: float = 100000.0, leverage: float = 1.0
    ):
        """
    ...
    def load_data(self, symbol: str, start_time: datetime, end_time: datetime):
        ...
        try:
            ...
            # Load tick data
            self.current_data = self.data_storage.load_tick_data(
    ...
    def _prepare_data(self):
        ...
        self.current_data["mid_price"] = (
            self.current_data["bid"] + self.current_data["ask"]
        ) / 2
        self.current_data["spread_bps"] = (
            (self.current_data["ask"] - self.current_data["bid"])
            / self.current_data["bid"]
        ) * 10000
        ...
        self.current_data["volatility"] = (
            self.current_data["mid_price"].rolling(window=20).std()
        )

    def _calculate_atr(self, period: int = 14):
        """Calculate Average True Range"""
        high_low = (
            self.current_data["bid"].rolling(window=period).max()
            - self.current_data["bid"].rolling(window=period).min()
        )
        high_close = np.abs(
            self.current_data["bid"].rolling(window=period).max()
            - self.current_data["bid"].shift(1)
        )
        low_close = np.abs(
            self.current_data["bid"].rolling(window=period).min()
            - self.current_data["bid"].shift(1)
    ...
    def step(self) -> Optional[MarketState]:
        ...
        if (
            ...
            or self.current_index >= len(self.current_data)
        ...
        market_state = MarketState(
            ...
            volatility=(
                float(tick.get("volatility", 0.0))
                if tick.get("volatility") is not None
                else 0.0
            ),
        ...
        try:
            ...
            try:
                ...
                return MarketUnderstanding(
                    ...
                    regime=(
                        str(synthesis_result.regime.value)
                        if hasattr(synthesis_result.regime, "value")
                        else str(synthesis_result.regime)
                    ),
                    narrative=str(synthesis_result.narrative),
                    anomaly_level=float(
                        synthesis_result.evidence.get("anomaly_level", 0.0)
                    ),
    ...
    def _validate_order(self, order: Order) -> bool:
        ...
        required_margin = (
            order.quantity
            * self.current_data.iloc[self.current_index]["bid"]
            / self.leverage
        )
        ...
        for order_id, order in list(self.orders.items()):
            ...
            if order.order_type == OrderType.MARKET:
                ...
                if order.side == OrderSide.BUY and current_tick["bid"] <= order.price:
                    ...
                    order.side == OrderSide.SELL and current_tick["ask"] >= order.price
                ):
                    self._execute_order(order_id)
                ...
                if order.side == OrderSide.BUY and current_tick["ask"] >= order.price:
                    ...
                    order.side == OrderSide.SELL and current_tick["bid"] <= order.price
                ):
                    self._execute_order(order_id)
        ...
        for symbol, position in self.positions.items():
            if position["quantity"] != 0:
                if position["quantity"] > 0:  # Long position
                    pnl = (market_state.mid_price - position["avg_price"]) * position[
                        "quantity"
                    ]
                else:  # Short position
                    pnl = (position["avg_price"] - market_state.mid_price) * abs(
                        position["quantity"]
                    )
    ...
    def _update_account(self):
        ...
        self.margin_used = sum(
            abs(pos["quantity"] * pos["avg_price"]) / self.leverage
            for pos in self.positions.values()
        )
...
class AdversarialEngine:
    ...
    def __init__(self, difficulty_level: float = 0.5, seed: Optional[int] = None):
        ...
        logger.info(
            f"Initialized adversarial engine with difficulty: {difficulty_level}"
        )

    def _create_config(self, difficulty: float) -> Dict:
        """Create configuration based on difficulty level"""
        return {
            # 1-10% chance per tick
            "event_frequency": 0.01 + (difficulty * 0.09),
            # 1-4 concurrent events
            "max_concurrent_events": int(1 + difficulty * 3),
    ...
    def update(self, market_state: MarketState, simulator: MarketSimulator):
        ...
        if (
            len(self.active_events) < self.config["max_concurrent_events"]
            and random.random() < self.config["event_frequency"]
        ):
    ...
    def _trigger_random_event(
        self, market_state: MarketState, simulator: MarketSimulator
    ):
        ...
        logger.info(
            f"Triggered {event_type.value} event with intensity {intensity:.2f}"
        )
    ...
    def _apply_spoofing_effects(
        self, market_state: MarketState, event: AdversarialEvent
    ):
    ...
    def _apply_stop_hunt_effects(
        ...
        price_move = (
            direction * event.intensity * market_state.atr * 0.5
            if market_state.atr
            else 0.001
        )
    ...
    def _apply_flash_crash_effects(
        self, market_state: MarketState, event: AdversarialEvent
    ):
