...
from typing import TYPE_CHECKING, Dict, List, Optional, Mapping, Protocol, TypeGuard, cast
...
class StrategyAnalyzerLegacy:
    ...
    async def analyze_behavior(self, target_strategy: Mapping[str, object],
                               test_scenarios: List[Dict[str, object]]) -> StrategyBehavior:
    ...
    async def _analyze_single_scenario(self, strategy: Mapping[str, object],
                                       scenario: Mapping[str, object]) -> Dict[str, object]:
    ...
    def _simulate_decision(self, strategy: Mapping[str, object],
                           scenario: Mapping[str, object], step: int) -> Dict[str, object]:
    ...
    def _calculate_outcome(self, decision: Mapping[str, object],
                           scenario: Mapping[str, object], step: int) -> float:
    ...
    def _create_performance_profile(self, behavior_data: List[Dict[str, object]]) -> Dict[str, float]:
        ...
        profit_factor = (abs(cast(float, np.sum(wins))) / max(abs(cast(float, np.sum(losses))), 0.001)) if wins else 0.0
...
class WeaknessDetectorLegacy:
    ...
    async def find_weaknesses(self, behavior_profile: StrategyBehavior,
                              known_vulnerabilities: List[Dict[str, object]]) -> List[Weakness]:
    ...
    def _check_vulnerability(self, behavior: StrategyBehavior,
                             vulnerability: Dict[str, object]) -> Optional[Weakness]:
...
class AttackGeneratorLegacy:
    ...
    async def create_attack(self, weakness: Weakness,
                            target_strategy: Mapping[str, object]) -> Attack:
    ...
    def _create_custom_attack(self, weakness: Weakness,
                              strategy: Mapping[str, object]) -> Attack:
    ...
    def _create_execution_plan(self, template: Mapping[str, object],
...
class ExploitDeveloperLegacy:
    ...
    async def develop_exploits(self, weaknesses: List[Weakness],
                               target_strategy: Mapping[str, object]) -> List[Dict[str, object]]:
    ...
    async def _develop_single_exploit(self, weakness: Weakness,
                                      strategy: Mapping[str, object]) -> Dict[str, object]:
    ...
    def _create_exploit_steps(self, template: Mapping[str, object],
...
class RedTeamAILegacy:
    ...
    def __init__(self) -> None:
        ...
        self.strategy_analyzer: StrategyAnalyzerLike = cast(StrategyAnalyzerLike, _StrategyAnalyzer()())
        self.weakness_detector: WeaknessDetectorLike = cast(WeaknessDetectorLike, _WeaknessDetector()())
        self.attack_generator: AttackGeneratorLike = cast(AttackGeneratorLike, _AttackGenerator()())
        self.exploit_developer: ExploitDeveloperLike = cast(ExploitDeveloperLike, _ExploitDeveloper()())
    ...
    async def _execute_attack(self, strategy: Mapping[str, object],
                              attack: Attack) -> Dict[str, object]:
    ...
    def _generate_recommendations(self, weaknesses: List[Weakness],
                                  results: List[Dict[str, object]]) -> List[str]:
    ...
    def _update_attack_history(self, strategy: Mapping[str, object],
    ...
    def get_attack_stats(self) -> Dict[str, object]:
        ...
        recent_attacks = [
            ...
            if isinstance(a.get('timestamp'), datetime) and cast(datetime, a['timestamp']) > week_ago
        ]
    
        avg_weaknesses = float(np.mean([_jf(a, 'weaknesses_found', 0.0) for a in recent_attacks])) if recent_attacks else 0.0
        avg_success_rate = float(np.mean([_jf(a, 'success_rate', 0.0) for a in recent_attacks])) if recent_attacks else 0.0
