...
- [ ] **Timeâ€‘toâ€‘candidate** â‰¤ 24h (idea â†’ scored in replay). _Progress: Findings memory now exposes SLA analytics and a metrics CLI reports average/median/p90 turnaround plus breach details, returning a non-zero exit code whenever SLA breaches persist so pipeline monitors can fail fast, while the new scheduler CLI quick-screens ideas, applies UCB-lite promotion with bounded `--max-quick`/`--max-full` budgets, and stamps evidence notes so candidates enter replay within the 24 h envelope under regression coverage.【F:emp/core/findings_memory.py†L1-L332】【F:emp/cli/emp_cycle_metrics.py†L1-L70】【F:emp/cli/emp_cycle_scheduler.py†L1-L200】【F:tests/emp_cycle/test_time_to_candidate.py†L1-L121】【F:tests/emp_cycle/test_cycle_scheduler.py†L1-L86】_
...
- [ ] **Attribution coverage** â‰¥ 90% (orders with belief + probes + brief explanation). _Progress: Loop runner telemetry now collapses rationale text into `_build_brief_explanation` payloads capped at 160 characters, preserves tactic/regime fallback summaries when rationale is empty, and continues to skip diary samples without belief IDs while bounding confidences so attribution payloads stay short and audited; the intent builder still propagates bounded coverage metadata on intents, and TradingManager now trims inbound explanations with the same limit before persisting them so downstream metrics see consistent brief text; diary exports carry bounded belief snapshots plus guardrail annotations, with coverage-check decision diaries and guard ledger refreshes capturing belief IDs, trimmed explanations, regime confidences, and approvals for promotion-check runs, while tests hammer long-rationale, fallback, executed, partial, and missing-attribution runs to keep warning semantics steady.【F:src/orchestration/alpha_trade_runner.py†L241-L347】【F:src/orchestration/alpha_trade_runner.py†L624-L904】【F:src/orchestration/alpha_trade_runner.py†L1094-L1223】【F:artifacts/diaries/2025/10/19/diary-224409/diary.json†L1-L80】【F:artifacts/diaries/2025/10/19/diary-224611/diary.json†L1-L80】【F:artifacts/diaries/2025/10/19/decision_diary-224059/decision_diary.json†L1-L111】【F:artifacts/diaries/2025/10/19/decision_guard-224059/decision_guard.json†L1-L145】【F:artifacts/ledger_exports/2025/10/19/policy_guard/policy_guard.json†L1-L29】【F:artifacts/ledger_exports/2025/10/19/policy_ledger/policy_ledger.json†L1-L51】【F:tests/orchestration/test_alpha_trade_runner.py†L241-L322】【F:tests/orchestration/test_alpha_trade_runner.py†L940-L1016】【F:src/trading/trading_manager.py†L1522-L1650】【F:src/trading/trading_manager.py†L3900-L3942】【F:tests/trading/test_trading_manager_execution.py†L1100-L1128】_
