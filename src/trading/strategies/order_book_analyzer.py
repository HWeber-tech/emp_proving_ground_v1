...
class OrderBookAnalyzer:
    ...
    def __init__(self, max_levels: int = 20, history_window: int = 1000):
        ...
        logger.info(f"Order book analyzer initialized with {max_levels} levels and {history_window} history window")
    
    def update_order_book(self, symbol: str, bids: List[Tuple[float, float]], 
                         asks: List[Tuple[float, float]], timestamp: Optional[datetime] = None) -> None:
        """
        ...
        logger.debug(f"Order book updated for {symbol}: spread={spread:.5f}, mid_price={mid_price:.5f}")
    
    def _analyze_microstructure(self, symbol: str, snapshot: OrderBookSnapshot) -> MarketMicrostructure:
        ...
        liquidity_imbalance = (bid_liquidity - ask_liquidity) / total_liquidity if total_liquidity > 0 else 0.0
    ...
    def get_liquidity_analysis(self, symbol: str, volume: float) -> Dict[str, Any]:
        ...
        available_bid_liquidity = sum(level.volume for level in 
                                    self.order_book_history[symbol][-1].bids[:5])
        available_ask_liquidity = sum(level.volume for level in 
                                    self.order_book_history[symbol][-1].asks[:5])
        ...
        return {
            ...
            'recommended_split': self._calculate_optimal_split(volume, available_bid_liquidity, available_ask_liquidity)
        }
    
    def _calculate_optimal_split(self, volume: float, bid_liquidity: float, ask_liquidity: float) -> Dict[str, float]:
    ...
    def get_microstructure_history(self, symbol: str, minutes: int = 60) -> List[MarketMicrostructure]:
