...
- **Progress**
  ...
  - AlphaTrade loop now injects belief/probe attribution into trade intents, persists diary coverage snapshots with gap alerts, and the trading manager records coverage metrics with warnings when attribution drops below the 90% goal so the roadmap's observability DoD has executable telemetry.【F:src/orchestration/alpha_trade_runner.py†L128-L420】【F:tests/orchestration/test_alpha_trade_runner.py†L660-L836】【F:src/trading/trading_manager.py†L3587-L3637】【F:tests/trading/test_trading_manager_execution.py†L760-L821】
