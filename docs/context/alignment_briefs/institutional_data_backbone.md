...
- Progress: A resilient WebSocket client with reconnect, heartbeat, and token-bucket throttling now ships alongside async pytest drills so upcoming venue adapters can stream ticks into the cache without rebuilding connectivity primitives.【F:src/data_foundation/streaming/websocket_client.py†L1-L200】【F:tests/data_foundation/streaming/test_websocket_client.py†L1-L200】
