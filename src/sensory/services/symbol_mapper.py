...
def _symbol_from_payload(info: Mapping[str, str | int | float]) -> SymbolInfo:
    """Coerce heterogeneous payload into SymbolInfo using safe numeric/string conversions."""
    return SymbolInfo(
        symbol_id=int(info.get("symbol_id", 0)),
        symbol_name=str(info.get("symbol_name", "")),
        description=str(info.get("description", "")),
        base_currency=str(info.get("base_currency", "")),
        quote_currency=str(info.get("quote_currency", "")),
...
class SymbolMapper:
    ...
    async def refresh_symbol_cache(self) -> None:
        ...
        try:
            ...
            # Cache in Redis
            symbol_map: Dict[str, Mapping[str, str | int | float]] = {
                symbol.symbol_name: {
            ...
            await self.state_store.set(
                self._cache_key, json.dumps(symbol_map), expire=self._cache_ttl
            )

            # Update local cache
            self._symbols = {name: _symbol_from_payload(info) for name, info in symbol_map.items()}
    ...
    async def _load_from_cache(self) -> None:
        ...
        try:
            ...
            if cached_data:
                symbol_map_loaded = json.loads(cached_data)
                self._symbols = {
                    name: _symbol_from_payload(cast(Mapping[str, str | int | float], info))
                    for name, info in symbol_map_loaded.items()
                }
