...
class DataHarmonizer:
    ...
    def harmonize_market_data(
        self, data_points: List[Tuple[DataSource, MarketData]]
    ) -> List[Tuple[DataSource, MarketData]]:
    ...
    def align_timeframes(
        self,
        data_points: List[Tuple[DataSource, MarketData]],
        target_interval: timedelta = timedelta(minutes=1),
    ) -> List[Tuple[DataSource, MarketData]]:
        ...
        aligned_data: Dict[datetime, List[Tuple[DataSource, MarketData]]] = {}
        for source, data in data_points:
...
class DataFusionEngine:
    ...
    async def fuse_market_data(
        self, symbol: str, data_providers: Dict[DataSource, Any]
    ) -> Optional[FusedDataPoint]:
        ...
        try:
            ...
            if self.config.enable_validation:
                validation = self.validator.validate_market_data(
                    resolved_data, self.config.validation_level
                )
                if not validation.is_valid:
                    logger.warning(
                        f"Fused data validation failed for {symbol}: {validation.issues}"
                    )
                    return None
            ...
            logger.info(
                f"Fused data for {symbol}: confidence={overall_confidence:.3f}, "
    ...
    async def _get_data_from_provider(
        self, provider: Any, symbol: str, source: DataSource
    ) -> Optional[MarketData]:
    ...
    def _calculate_overall_confidence(
        ...
        if total_weight == 0:
            return float(np.mean([conf for _, _, conf in data_points]))
...
async def test_data_fusion() -> None:
