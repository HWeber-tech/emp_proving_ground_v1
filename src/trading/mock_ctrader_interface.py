...
class MockCTraderInterface:
    ...
    async def connect(self) -> bool:
        ...
        try:
            ...
            # Create mock client
            self.client = MockCTraderClient(
                self.config.host, self.config.port, self._on_message_received
            )
    ...
    async def place_order(
        ...
        try:
            ...
            logger.info(
                f"Order placed: {side.value} {volume} {symbol_name} (ID: {order_id})"
            )
            return order_id
    ...
    def _generate_mock_market_data(self, symbol_id: int, symbol_name: str):
        ...
        base_price = (
            ...
            else 1.25123 if symbol_name == "GBPUSD" else 150.123
        )
    ...
    async def _simulate_order_fill(self, order_id: str, symbol_name: str):
        ...
        current_price = (
            self.market_data[order.symbol_id].bid
            + self.market_data[order.symbol_id].ask
        ) / 2
