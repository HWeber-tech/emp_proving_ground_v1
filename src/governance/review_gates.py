...
@dataclass(frozen=True, slots=True)
class AutoRFCProposal:
    """Patch proposal capturing remediation steps for a failing gate."""
...
class ReviewGateRegistry:
    ...
    def _build_auto_rfc_proposal(
        self,
        entry: ReviewGateEntry,
        *,
        generated_at: datetime,
        base_directory: Path,
    ) -> AutoRFCProposal:
        ...
        if decision is None:
            raise ValueError("auto RFC proposals require a recorded decision")

        definition = entry.definition
        failed_criteria: list[ReviewGateCriterion] = []
        remediation_actions: list[str] = []
        for criterion in definition.criteria:
            status = decision.criteria_status.get(criterion.criterion_id)
            if status is ReviewCriterionStatus.not_met:
                failed_criteria.append(criterion)
                remediation_actions.append(
                    f"- Restore `{criterion.criterion_id}` compliance: {criterion.description}."
                )

        failure_reasons: list[str] = []
        if decision.verdict is ReviewVerdict.fail:
            failure_reasons.append("Gate recorded a FAIL verdict.")
        for criterion in failed_criteria:
            failure_reasons.append(
                f"Criterion `{criterion.criterion_id}` is not met ({criterion.description})."
            )
        if not failure_reasons:
            failure_reasons.append("Gate requires remediation before promotion can continue.")

        summary = (
            f"{definition.title} gate is blocked (severity {definition.severity}). "
            + " ".join(failure_reasons)
        ).strip()

        if not remediation_actions:
            remediation_actions.append(
                "- Review the recorded notes and document concrete remediation steps before re-running the gate."
            )
        remediation_actions.append(
            "- Update the gate decision to PASS or WARN once remediation evidence is captured."
        )

        proposal_path = base_directory / f"auto_rfc_{definition.gate_id}.md"
        header = f"# RFC: Remediate {definition.title}"

        lines: list[str] = [header, "", "## Summary", summary, "", "## Proposed Remediation"]
        lines.extend(remediation_actions)
        lines.append("")
        lines.append("## Gate Metadata")
        lines.append(f"- Gate ID: `{definition.gate_id}`")
        lines.append(f"- Severity: {definition.severity}")
        lines.append(f"- Verdict: {decision.verdict.value}")
        lines.append(f"- Decided at: {decision.decided_at.isoformat()}")
        ...
        if definition.artifacts:
            lines.append("- Artifacts: " + ", ".join(definition.artifacts))
        if failed_criteria:
            lines.append("- Failed criteria: " + ", ".join(f"`{item.criterion_id}`" for item in failed_criteria))
        lines.append("")
        if decision.notes:
            ...
            for note in decision.notes:
                lines.append(f"- {note}")
            lines.append("")
        lines.append("## Auto-RFC Metadata")
        lines.append(f"- Generated at: {generated_at.isoformat()}")
        lines.append("- Generated by: ReviewGateRegistry.auto_rfc_proposals")
    ...
    def auto_rfc_proposals(
        self,
        *,
        generated_at: datetime | None = None,
        base_directory: Path | None = None,
    ) -> tuple[AutoRFCProposal, ...]:
        timestamp = generated_at or datetime.now(tz=UTC)
        output_dir = base_directory or Path("docs/rfcs/auto")
        proposals: list[AutoRFCProposal] = []
        for entry in self.iter_entries():
            if entry.status() is not WorkflowTaskStatus.blocked:
                continue
            decision = entry.decision
            if decision is None:
                continue
            proposals.append(
                self._build_auto_rfc_proposal(
                    entry,
    ...
    def to_summary(self, *, generated_at: datetime | None = None) -> dict[str, object]:
        summary_generated_at = generated_at or datetime.now(tz=UTC)
        ...
        return {
            ...
            "auto_rfc_proposals": auto_rfc_payloads,
        }
