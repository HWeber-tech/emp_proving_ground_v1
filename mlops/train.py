...
class ModelTrainer:
    ...
    def rolling_origin_validation(self, X: np.ndarray, y: np.ndarray, 
                                n_splits: int = 5, 
                                train_size: int = 500,
        ...
        for fold in range(n_splits):
            ...
            logger.info(f"Fold {fold + 1}: Train {train_idx.start}-{train_idx.stop}, Test {test_idx.start}-{test_idx.stop}")
    ...
    def train_model(self, X_train: np.ndarray, y_train: np.ndarray,
        ...
        for epoch in range(epochs):
            ...
            if epoch % 10 == 0:
                logger.info(f"Epoch {epoch}: Train Acc: {train_accuracy:.2f}%, Test Acc: {test_accuracy:.2f}%")
        ...
        with mlflow.start_run():
            ...
            for fold_data in cv_results:
                fold_result = self.train_model(
                    ...
                    fold_data['X_test'], fold_data['y_test'],
                    feature_names, fold_data['fold']
                )
...
def main():
    ...
    parser.add_argument('--data-path', default='data/training/v1_training_dataset.parquet', help='Path to training data')
