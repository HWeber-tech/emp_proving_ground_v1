...
from src.core.events import LearningSignal, ContextPacket
...
class MetaCognitionEngine:
    ...
    async def assess_learning_quality(
        ...
        try:
            ...
            logger.debug(
                f"Learning quality assessment: {overall_quality:.2f}, "
                f"Adapt: {should_adapt}"
            )
    ...
    async def _calculate_accuracy_score(self, learning_signal: LearningSignal) -> float:
        ...
        try:
            ...
            predicted_outcomes = [p['predicted_pnl'] for p in historical_predictions]
            actual_outcomes = [p['actual_pnl'] for p in historical_predictions]
            ...
            mae = sum(abs(p - a) for p, a in zip(predicted_outcomes, actual_outcomes)) / len(predicted_outcomes)
    ...
    async def _calculate_consistency_score(self, learning_signal: LearningSignal) -> float:
        ...
        try:
            ...
            outcomes = [float(s.outcome_pnl) for s in recent_signals]
    ...
    async def _calculate_relevance_score(self, learning_signal: LearningSignal) -> float:
        ...
        try:
            ...
            # Combine scores
            relevance_score = (
                context_stability * 0.4 +
                regime_relevance * 0.3 +
                confidence_relevance * 0.3
            )
    ...
    async def _calculate_context_stability(self, context: ContextPacket) -> float:
        ...
        try:
            ...
            confidence_changes = [
                abs(float(c.confidence) - float(context.confidence))
    ...
    async def calibrate_confidence(
        ...
        try:
            ...
            # Apply calibration
            calibrated_confidence = float(learning_signal.confidence_of_outcome) * calibration_factor
