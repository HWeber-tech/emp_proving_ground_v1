...
class StrategyPerformance:
    ...
    def update_from_trade(self, trade_pnl: float):
        ...
        self.avg_win = (
            self.total_profit / self.winning_trades if self.winning_trades > 0 else 0
        )
        self.avg_loss = (
            self.total_loss / self.losing_trades if self.losing_trades > 0 else 0
        )
...
class StrategyManager:
    ...
    def __init__(self, strategies_dir: str = "strategies", max_strategies: int = 10):
        ...
        # 'performance_ranked', 'ensemble', 'regime_based'
        self.selection_method = "performance_ranked"
    ...
    def add_strategy(self, genome: DecisionGenome) -> bool:
        ...
        try:
            ...
            logger.info(
                f"Added strategy {strategy_id} (generation {genome.generation})"
            )
            return True
            ...
            for strategy_id, genome in self.strategies.items():
                try:
                    signal = self._evaluate_strategy(
                        genome, symbol, sensory_reading, market_data
                    )
    ...
    def update_strategy_performance(self, strategy_id: str, trade_pnl: float):
        ...
        if strategy_id in self.performance:
            ...
            logger.debug(
                f"Updated performance for strategy {strategy_id}: P&L {trade_pnl:.2f}"
            )
    ...
    def get_top_strategies(
        self, count: int = 5
    ) -> List[Tuple[str, StrategyPerformance]]:
        ...
        sorted_strategies = sorted(
            self.performance.items(), key=lambda x: x[1].net_profit, reverse=True
        )
    ...
    def get_strategy_summary(self) -> Dict[str, Any]:
        ...
        active_strategies = sum(
            1 for perf in self.performance.values() if perf.total_trades > 0
        )
    ...
    def _calculate_confidence(self, sensory_reading, genome: DecisionGenome) -> float:
        ...
        try:
            ...
            if hasattr(sensory_reading, "institutional_confidence"):
                strength_multiplier *= (
                    0.5 + sensory_reading.institutional_confidence * 0.5
                )
    ...
    def _calculate_stop_loss(
        self, entry_price: float, decision: str, genome: DecisionGenome
    ) -> float:
    ...
    def _get_sensory_reading(self, market_data: Dict[str, Any]):
        ...
        try:
            ...
            sensory_data = MarketData(
                ...
                bid=market_data.get(
                    "bid", market_data.get("low", market_data.get("close", 1.1000))
                ),
                ask=market_data.get(
                    "ask", market_data.get("high", market_data.get("close", 1.1002))
                ),
                source="live",
                latency_ms=0.0,
            )
    ...
    def _select_by_performance_ranking(
    ...
    def _select_by_ensemble(
        ...
        if buy_signals and sell_signals:
            ...
            sell_confidence = sum(s.confidence for s in sell_signals) / len(
                sell_signals
            )
    ...
    def _select_by_regime(
        self, signals: List[StrategySignal], market_data: Dict[str, Any]
    ) -> Optional[StrategySignal]:
        ...
        try:
            for strategy_file in self.strategies_dir.glob("*.pkl"):
                try:
                    ...
                    strategy_id = genome.genome_id
                    self.strategies[strategy_id] = genome
                    self.performance[strategy_id] = StrategyPerformance(
    ...
    def _remove_worst_strategy(self):
        ...
        worst_strategy = min(self.performance.items(), key=lambda x: x[1].net_profit)
...
def main():
    ...
    parser.add_argument(
        "--test-evaluation", action="store_true", help="Test strategy evaluation"
    )
    parser.add_argument(
        "--test-selection", action="store_true", help="Test strategy selection"
    )
    ...
    if args.test_evaluation:
        ...
        for signal in signals:
            print(
                f"Signal: {signal.action} {signal.symbol} (confidence: {signal.confidence:.2%})"
            )
    ...
    if args.test_selection:
        ...
        if best_signal:
            print(
                f"Best signal: {best_signal.action} {best_signal.symbol} (confidence: {best_signal.confidence:.2%})"
            )
        else:
