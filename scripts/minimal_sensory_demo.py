...
class MinimalDimensionalEngine:
    ...
    async def get_reading(self, market_data: MarketData) -> DimensionalReading:
        ...
        reading = DimensionalReading(
            ...
            regime=MarketRegime.TRENDING_STRONG if abs(signal_strength) > 0.3 else MarketRegime.CONSOLIDATING,
            context=context,
...
class MinimalSensoryDemo:
    ...
    async def run_demo(self):
        ...
        print("Tick     | Time     | WHY    | HOW    | WHAT   | WHEN   | ANOMALY | Unified | Confidence")
        print("-" * 90)
        ...
        for dim_name, reading in self.fusion_engine.current_readings.items():
            print(f"  {dim_name:8s}: {reading.signal_strength:+.3f} (confidence: {reading.confidence:.3f})")
