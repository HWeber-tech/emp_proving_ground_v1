...
- Progress: Release-aware execution router now reconfigures pilot and live
  ...
  - Progress: Trading manager now installs the PaperBrokerExecutionAdapter as the limited-live engine with stage defaults, risk-context snapshots, and broker failure capture so decision diaries persist the last error payload when simulations falter. Bootstrap can attach a REST `PaperTradingApiAdapter` when extras are supplied, exercising real HTTP round-trips, cleanup callbacks, and failure retries under pytest coverage so paper pilots inherit deterministic routing without bespoke wiring, and a dedicated simulation runner/CLI now drives the bootstrap runtime end-to-end to capture broker orders, errors, and diary counts for roadmap evidence packs.【src/trading/execution/paper_broker_adapter.py†L1-L310】【src/orchestration/bootstrap_stack.py†L418-L462】【src/runtime/predator_app.py†L2087-L2149】【src/runtime/paper_simulation.py†L1-L156】【tools/trading/run_paper_trading_simulation.py†L1-L198】【tests/runtime/test_bootstrap_paper_broker.py†L1-L147】【tests/runtime/test_paper_trading_simulation_runner.py†L39-L94】
- Progress: Execution pipeline now enforces configurable trade throttles via the trading manager, layering scope-aware windows, minimum spacing guards, and retry metadata so bursty strategies respect governance cooldowns, with regression coverage spanning throttle helpers, trading-manager snapshots, and operator docs.【F:src/trading/execution/trade_throttle.py†L18-L344】【F:src/trading/trading_manager.py†L333-L454】【F:docs/performance/trading_throughput_monitoring.md†L87-L116】【F:tests/trading/test_trade_throttle.py†L1-L147】【F:tests/trading/test_trading_manager_execution.py†L1730-L1899】
  - Progress: Throughput monitoring now layers backlog and resource snapshots into `TradingManager.assess_throughput_health()`, exposing event-loop lag, breach counts, and CPU/memory envelopes so ops checks can assert latency and capacity budgets from a single snapshot during high-frequency drills under refreshed regression coverage.【F:src/trading/execution/performance_monitor.py†L1-L143】【F:src/trading/execution/backlog_tracker.py†L1-L102】【F:src/trading/execution/resource_monitor.py†L1-L109】【F:src/trading/trading_manager.py†L1237-L1335】【F:docs/performance/trading_throughput_monitoring.md†L43-L118】【F:tests/trading/execution/test_backlog_tracker.py†L1-L42】【F:tests/trading/execution/test_resource_monitor.py†L1-L52】【F:tests/trading/test_trading_manager_execution.py†L1756-L1895】
