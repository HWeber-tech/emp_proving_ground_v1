...
class RiskManager:
    ...
    def _calculate_pip_value(self, instrument: Instrument, account_currency: str) -> Decimal:
        """
    ...
    def validate_order(self, proposed_order: 'Order', account_state: Dict, 
        ...
        if not instrument:
            return ValidationResult(
                is_valid=False,
                reason=f"Instrument {proposed_order.symbol} not found"
            )
        ...
        if not self._validate_leverage(proposed_order, account_state, open_positions):
            return ValidationResult(
                is_valid=False,
                reason="Order exceeds maximum leverage limits"
            )
        ...
        if not self._validate_exposure(proposed_order, account_state, open_positions):
            return ValidationResult(
                is_valid=False,
                reason="Order exceeds maximum exposure limits"
            )
        ...
        if not self._validate_drawdown(account_state):
            return ValidationResult(
                is_valid=False,
                reason="Account drawdown exceeds maximum allowed"
            )
        ...
        if not self._validate_position_size(proposed_order):
            return ValidationResult(
                is_valid=False,
                reason="Position size outside allowed limits"
            )
        ...
        return ValidationResult(
            ...
            risk_metadata={
                'leverage_used': self._calculate_leverage_used(proposed_order, account_state, open_positions),
                'exposure_pct': self._calculate_exposure_pct(proposed_order, account_state, open_positions),
                'risk_per_trade_pct': self._calculate_risk_per_trade_pct(proposed_order, account_state)
            }
        )
    
    def _validate_leverage(self, order: 'Order', account_state: Dict, 
                          open_positions: Dict[str, 'EnhancedPosition']) -> bool:
        """Validate leverage limits."""
        current_leverage = self._calculate_leverage_used(order, account_state, open_positions)
        return current_leverage <= self.config.max_leverage
    
    def _validate_exposure(self, order: 'Order', account_state: Dict, 
                          open_positions: Dict[str, 'EnhancedPosition']) -> bool:
        """Validate exposure limits."""
        exposure_pct = self._calculate_exposure_pct(order, account_state, open_positions)
    ...
    def _validate_position_size(self, order: 'Order') -> bool:
        ...
        return (self.config.min_position_size <= abs(order.quantity) <= 
                self.config.max_position_size)
    
    def _calculate_leverage_used(self, order: 'Order', account_state: Dict, 
        ...
        if instrument:
            total_exposure += Decimal(str(abs(order.quantity))) * instrument.contract_size
        
        # Add exposure from existing positions
        for position in open_positions.values():
            instrument = self.instrument_provider.get_instrument(position.symbol)
            if instrument:
                total_exposure += Decimal(str(abs(position.quantity))) * instrument.contract_size
    ...
    def _calculate_risk_per_trade_pct(self, order: 'Order', account_state: Dict) -> Decimal:
