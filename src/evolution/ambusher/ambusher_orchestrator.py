...
class AmbusherOrchestrator:
    ...
    async def start(self) -> None:
        ...
        if self.genome_path.exists():
            # Some engines may not expose load_genome; use cast to avoid type errors
            self.current_genome = cast(Any, self.genetic_engine).load_genome(str(self.genome_path))
    ...
    async def evolve_strategy(
        self, market_data: Dict[str, Any], trade_history: List[Dict[str, Any]]
    ) -> Dict[str, Any]:
        ...
        # Evolve without capturing a non-existent return value
    ...
    def get_current_strategy(self) -> Optional[Dict[str, Any]]:
        ...
        if self.current_genome:
            if hasattr(self.current_genome, "to_dict"):
                return cast(Dict[str, Any], cast(Any, self.current_genome).to_dict())
