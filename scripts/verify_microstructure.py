...
class MicrostructureVerifier:
    ...
    async def _get_symbols(self, ws):
        ...
        await ws.send(
            json.dumps({"payloadType": 2120, "payload": {"ctidTraderAccountId": self.account_id}})
        )
    ...
    async def _collect_depth_data(self, ws):
        ...
        if data.get("payloadType") == 2126:
            ...
            self.results["sample_data"] = {
                ...
                "bids": [
                    {"price": b.get("price", 0) / 100000, "volume": b.get("volume", 0) / 100}
                    for b in bids[:3]
                ],
                "asks": [
                    {"price": a.get("price", 0) / 100000, "volume": a.get("volume", 0) / 100}
                    for a in asks[:3]
                ],
            }
