...
- [ ] `[adapt]` **Counterfactual guardrails** (passive vs aggro delta bounds) for live candidates. _Progress: AlphaTradeLoopRunner now merges counterfactual guardrail payloads with risk metadata so guardrail snapshots flow into trade metadata, outcomes, diaries, and intents, while the release router forces paper only when severity or explicit actions demand it, keeping near-miss noise from forcing routes under regression coverage.【F:src/orchestration/alpha_trade_runner.py†L245-L343】【F:src/orchestration/alpha_trade_runner.py†L666-L705】【F:tests/orchestration/test_alpha_trade_runner.py†L262-L333】【F:tests/orchestration/test_alpha_trade_runner.py†L338-L419】【F:src/trading/execution/release_router.py†L453-L507】_
...
- [ ] `[core]` Live market data ingest configured (API keys, symbols, session calendars). _Progress: Institutional ingest builder now surfaces API key posture, FX session calendars, macro calendars, and enriched symbol inventories (contract size, margin currency, swap timing) via ReferenceDataLoader metadata, with regression coverage confirming London FX schedules and margin instrumentation while live wiring remains outstanding; managed ingest connectors CLI now exports operator summaries for symbols, inventory, API keys, and calendars so operators receive margin-aware manifests under coverage verifying London FX calendars and inventory projections.【F:src/data_foundation/ingest/configuration.py†L74-L200】【F:src/data_foundation/ingest/configuration.py†L893-L934】【F:tests/data_foundation/test_timescale_config.py†L292-L326】【F:tools/operations/managed_ingest_connectors.py†L403-L519】【F:tests/tools/test_managed_ingest_connectors.py†L42-L82】_
