...
from .base import ConfidenceLevel, MarketRegime
...
class ConfidenceFactors:
    ...
    historical_accuracy: float = 1.0  # Historical performance of this signal (0-1)
    market_conditions: float = 1.0    # Suitability of current market conditions (0-1)
    confluence: float = 1.0           # Agreement with other indicators (0-1)
    volatility_adjustment: float = 1.0  # Adjustment for current volatility (0-1)
...
def compute_confidence(
    ...
    try:
        ...
        if confluence_signals:
            ...
            factors.confluence = np.mean(signal_agreements) if signal_agreements else 0.5
        else:
        ...
        logger.debug(f"Confidence calculation: signal={signal_strength:.3f}, "
...
def calculate_divergence(price_series: List[float], indicator_series: List[float]) -> float:
    ...
    if price_trend * indicator_trend < 0:
        ...
        return abs(price_trend + indicator_trend) / 2  # Positive for confluence
...
class PerformanceTracker:
    ...
    def __init__(self, window_size: int = 100):
        ...
        self.predictions: List[Tuple[float, float, bool]] = []  # (signal, confidence, correct)
        
    def add_prediction(self, signal: float, confidence: float, actual_outcome: bool) -> None:
        """
    ...
    def get_accuracy(self, min_confidence: float = 0.0) -> float:
        ...
        filtered_predictions = [
            (signal, conf, correct) for signal, conf, correct in self.predictions
