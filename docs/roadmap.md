...
- [ ] `[obs]` Attribution coverage â‰¥ **90%** of orders have belief + probes; no Ïƒ explosions (bounded norms). _Progress: AlphaTradeLoopRunner now only attaches bounded confidences when belief metadata or regime state provides them, normalises metadata overrides, and keeps probe/explanation fallbacks so attribution payloads avoid zeroed confidences while still capturing top features; the trade intent builder continues to thread bounded coverage metadata into trades and intents, and TradingManager still postpones denominators until belief/probe cores exist so partial payloads no longer erode the coverage metric; fresh diaries confirm belief IDs, bounded confidences, and probes land under the sanitised schema with guardrail annotations while regression coverage drives executed, partial, and missing-attribution sequences to keep warning semantics steady.【F:src/orchestration/alpha_trade_runner.py†L241-L347】【F:src/orchestration/alpha_trade_runner.py†L600-L704】【F:artifacts/diaries/2025/10/19/diary-224409/diary.json†L1-L80】【F:artifacts/diaries/2025/10/19/diary-224611/diary.json†L1-L80】【F:tests/orchestration/test_alpha_trade_runner.py†L241-L320】【F:tests/orchestration/test_alpha_trade_runner.py†L940-L1016】【F:src/trading/trading_manager.py†L1522-L1650】【F:src/trading/trading_manager.py†L4030-L4122】【F:tests/trading/test_trading_manager_execution.py†L1200-L1229】_
...
- [ ] `[core]` Live market data ingest configured (API keys, symbols, session calendars). _Progress: Institutional ingest builder now caches ReferenceDataLoader instrument inventory alongside API key posture, FX session calendars, macro calendars, and symbol metadata (contract size, pip decimals, margin currency, swap timing) for the configured symbol list, short-circuits to an empty inventory when no symbols are configured so unprovisioned environments never leak the full catalog, and still constrains the exported inventory to only configured symbols so manifests omit unsubscribed pairs, while managed ingest connectors CLI surfaces both inventory and metadata snapshots with swap rate context so operators receive margin-aware manifests; regression coverage confirms metadata exports for the London FX calendar, EURUSD-only inventory, and CLI wiring while live wiring remains outstanding.【F:src/data_foundation/ingest/configuration.py†L199-L220】【F:src/data_foundation/ingest/configuration.py†L955-L976】【F:tests/data_foundation/test_timescale_config.py†L320-L327】【F:tools/operations/managed_ingest_connectors.py†L420-L473】【F:tests/tools/test_managed_ingest_connectors.py†L60-L66】_
...
- [ ] **Attribution coverage** â‰¥ 90% (orders with belief + probes + brief explanation). _Progress: Loop runner telemetry continues to normalise attribution payloads by skipping diary samples without belief IDs, trimming probe metadata, bounding confidences, and injecting fallback explanations, and now only attaches bounded confidences when the belief or regime supplies them while rebinding metadata overrides so payloads avoid zeroed confidences; the intent builder still propagates bounded coverage metadata on intents, and TradingManager keeps delaying denominators until belief/probe cores are present so partial payloads no longer drag the metric below target; diary exports carry bounded belief snapshots plus guardrail annotations, with the latest coverage-check decision diaries and guard ledger refreshes capturing belief IDs, regime confidences, and approvals for promotion-check runs to keep the evidence trail current, and tests drive executed, partial, and missing-attribution runs to keep warning semantics steady.【F:src/orchestration/alpha_trade_runner.py†L241-L347】【F:src/orchestration/alpha_trade_runner.py†L600-L704】【F:src/orchestration/alpha_trade_runner.py†L1094-L1223】【F:artifacts/diaries/2025/10/19/diary-224409/diary.json†L1-L80】【F:artifacts/diaries/2025/10/19/diary-224611/diary.json†L1-L80】【F:artifacts/diaries/2025/10/19/decision_diary-224059/decision_diary.json†L1-L111】【F:artifacts/diaries/2025/10/19/decision_guard-224059/decision_guard.json†L1-L145】【F:artifacts/ledger_exports/2025/10/19/policy_guard/policy_guard.json†L1-L29】【F:artifacts/ledger_exports/2025/10/19/policy_ledger/policy_ledger.json†L1-L51】【F:tests/orchestration/test_alpha_trade_runner.py†L241-L320】【F:tests/orchestration/test_alpha_trade_runner.py†L940-L1016】【F:src/trading/trading_manager.py†L1522-L1650】【F:src/trading/trading_manager.py†L4030-L4122】【F:tests/trading/test_trading_manager_execution.py†L1200-L1229】_
