...
from market_intelligence.orchestration.enhanced_intelligence_engine import \
...
class MarketDataSimulator:
        ...
        for i in range(24):
            ...
            if np.random.random() < 0.1:  # 10% chance per hour
                event = {
                    ...
                    # Slightly more volatile than expected
                    "actual": np.random.normal(0, 1.2),
                }
                events.append(event)
    ...
    def _update_market_regime(self):
        ...
        if np.random.random() < transition_prob:
            ...
            # Slightly favor ranging markets
...
class PerformanceMonitor:
    ...
    def record_analysis(
        self, analysis_time: float, synthesis: Any, memory_mb: float = None
    ):
    ...
    def get_performance_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "avg_analysis_time": (
                np.mean(self.metrics["analysis_times"])
                if self.metrics["analysis_times"]
                else 0
            ),
            "max_analysis_time": (
                np.max(self.metrics["analysis_times"])
                if self.metrics["analysis_times"]
                else 0
            ),
            "throughput": (
                len(self.metrics["analysis_times"]) / total_time
                if total_time > 0
                else 0
            ),
            "avg_confidence": (
                np.mean(self.metrics["confidence_levels"])
                if self.metrics["confidence_levels"]
                else 0
            ),
            "avg_unified_score": (
                np.mean(self.metrics["synthesis_scores"])
                if self.metrics["synthesis_scores"]
                else 0
            ),
            "score_volatility": (
                np.std(self.metrics["synthesis_scores"])
                if self.metrics["synthesis_scores"]
                else 0
            ),
            "memory_usage": {
                "avg": (
                    np.mean(self.metrics["memory_usage"])
                    if self.metrics["memory_usage"]
                    else 0
                ),
                "max": (
                    np.max(self.metrics["memory_usage"])
                    if self.metrics["memory_usage"]
                    else 0
                ),
            },
...
class IntelligenceDemo:
    ...
    async def run_comprehensive_demo(self):
        ...
        for cycle in range(self.demo_duration):
            ...
            synthesis = await self.fusion_engine.analyze_market_intelligence(
                market_data
            )
            ...
            if cycle % self.display_interval == 0:
                self._display_basic_results(
                    cycle, market_data, synthesis, analysis_time
                )
    ...
    def _display_basic_results(
        self, cycle: int, market_data: MarketData, synthesis: Any, analysis_time: float
    ):
        """Display basic analysis results"""

        print(
            f"Cycle {cycle:3d} | Price: {(market_data.bid + market_data.ask)/2:.5f} | "
    ...
    async def _display_detailed_analysis(self, cycle: int):
        ...
        for dimension, reading in self.fusion_engine.current_readings.items():
            print(
                f"  {dimension:8s}: {reading.value:+.3f} (confidence: {reading.confidence:.3f})"
            )
        print()
        ...
        # Correlations
        correlations = (
            self.fusion_engine.correlation_analyzer.get_dimensional_correlations()
        )
        if correlations:
            print("DIMENSIONAL CORRELATIONS:")
            # Top 5
            for (dim_a, dim_b), corr in list(correlations.items())[:5]:
                print(
                    f"  {dim_a}-{dim_b}: {corr.correlation:+.3f} (sig: {corr.significance:.3f})"
                )
            print()

        # Patterns
        patterns = (
            self.fusion_engine.correlation_analyzer.get_cross_dimensional_patterns()
        )
        if patterns:
            print("DETECTED PATTERNS:")
            for pattern in patterns[:3]:  # Top 3
                print(
                    f"  • {pattern.pattern_name}: {pattern.pattern_strength:.3f} confidence"
                )
            print()
    ...
    async def _display_final_summary(self):
        ...
        print(
            f"  Average Analysis Time: {perf_summary['avg_analysis_time']*1000:.1f}ms"
        )
        print(
            f"  Maximum Analysis Time: {perf_summary['max_analysis_time']*1000:.1f}ms"
        )
        print()
        ...
        if "adaptive_weights" in diagnostics:
            for dimension, weight_info in diagnostics["adaptive_weights"].items():
                if dimension != "current_regime":
                    ...
                    print(
                        f"  {dimension:8s}: Weight={current_weight:.3f}, Accuracy={accuracy:.3f}"
                    )
        print()
        ...
        if "correlations" in diagnostics and diagnostics["correlations"]:
            ...
            for pair, corr_info in sorted_corrs[:5]:
                ...
                print(
                    f"  {pair}: {correlation:+.3f} (significance: {significance:.3f})"
                )
            print()
        ...
        print(
            "The multidimensional market intelligence system is ready for production use."
        )
        print("=" * 80)
...
class InteractiveDemo:
    ...
    async def _stream_analysis(self, count: int):
        ...
        for i in range(count):
            ...
            print(
                f"{i+1:5d} | {price:.5f} | {synthesis.intelligence_level.name:12s} | "
    ...
    def _show_diagnostics(self):
        ...
        if "current_readings" in diagnostics:
            ...
            for dimension, reading in diagnostics["current_readings"].items():
                print(
                    f"  {dimension:8s}: {reading['value']:+.3f} (conf: {reading['confidence']:.3f})"
                )
            print()
