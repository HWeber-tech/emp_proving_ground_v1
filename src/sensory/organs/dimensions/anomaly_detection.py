...
class ManipulationDetectionSystem:
    ...
    async def detect_manipulation(self, market_data: pd.DataFrame) -> AnomalyDetection:
        ...
        try:
            ...
            microstructure_anomalies = await self.microstructure_analyzer.detect_anomalies(
                market_data
            )

            # Calculate overall risk score
            overall_risk = self._calculate_overall_risk(
                spoofing, wash_trading, pump_dump, microstructure_anomalies
            )
    ...
    def _calculate_overall_risk(
        ...
        wash_trading: WashTradingDetection,
        pump_dump: PumpDumpDetection,
        microstructure: list[dict[str, Any]],
    ) -> float:
        """Calculate overall risk score"""
        risk_factors = [
        ...
        return float(min(max(float(np.mean(risk_factors)), 0.0), 1.0))
    ...
    def _calculate_detection_confidence(
        ...
        confidences = [spoofing.confidence, wash_trading.confidence, pump_dump.confidence]
...
class SpoofingDetector:
    ...
    def _detect_price_anomaly(self, data: pd.DataFrame) -> bool:
        ...
        recent_change: float = float(
            abs(float(prices.iloc[-1]) - float(prices.iloc[-5])) / float(prices.iloc[-5])
        )
        return bool(recent_change > 0.05)
...
class PumpDumpDetector:
    ...
    def _calculate_price_change(self, data: pd.DataFrame) -> float:
        ...
        if start == 0.0:
            return 0.0
        return float(abs(end - start) / start)
                        ...
                        anomalies.append(
                            {
                                "type": "wide_spread",
                                "timestamp": (
                                    data.index[i] if hasattr(data, "index") else datetime.now()
                                ),
