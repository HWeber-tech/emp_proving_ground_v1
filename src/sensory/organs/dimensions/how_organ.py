...
class HowEngine:
    ...
    def __init__(self, config: Optional[dict[str, Any]] = None):
        ...
        # Predeclare attributes as optional to allow None fallback on ImportError
        self.indicators: TechnicalIndicators | None
        self.patterns: ICTPatternDetector | None
        self.order_flow_provider: OrderFlowDataProvider | None
        self.order_flow_analyzer: OrderFlowAnalyzer | None
        self.microstructure: MarketMicrostructureAnalyzer | None
        try:
            from .indicators import TechnicalIndicators
            from .order_flow import MarketMicrostructureAnalyzer, OrderFlowAnalyzer
            from .patterns import ICTPatternDetector, OrderFlowDataProvider
    ...
    def analyze_market_data(
        ...
        try:
            ...
            analysis_results = {
                ...
                "institutional_mechanics": self.analyze_institutional_mechanics(
                    market_data, symbol
                ),
            }
    ...
    def analyze_institutional_mechanics(
        self, market_data: list[MarketData], symbol: str = "UNKNOWN"
    ) -> dict[str, Any]:
        """
    ...
    def _get_institutional_pressure(self, df: pd.DataFrame) -> dict[str, float]:
        ...
        try:
            flow_analysis: dict[str, Any] = self.order_flow_analyzer.analyze_institutional_flow(df)
            default = {"buying_pressure": 0.0, "selling_pressure": 0.0}
            raw = flow_analysis.get("institutional_pressure", default)
            if isinstance(raw, dict):
                buying = float(raw.get("buying_pressure", 0.0))
                selling = float(raw.get("selling_pressure", 0.0))
                return {"buying_pressure": buying, "selling_pressure": selling}
            return default
        except Exception as e:
    ...
    def _calculate_signal_strength(self, analysis: dict[str, Any]) -> float:
        ...
        try:
            ...
            return float(min(max(float(signal_strength), -1.0), 1.0))
    ...
    def _calculate_confidence(self, analysis: dict[str, Any]) -> float:
        ...
        try:
            ...
            return float(min(max(float(base_confidence), 0.0), 1.0))
