...
def test_generate_risk_report_includes_expected_metrics(sample_returns: list[float]) -> None:
    ...
    exposures = {"EURUSD": 2.5, "GBPUSD": 1.2, "AAPL": -1.1}
    ...
    assert "sector_limits" in report.breaches
    assert "asset_class_limits" in report.breaches
    sector_breach = report.breaches["sector_limits"]
    assert "macro_fx" in sector_breach
    asset_breach = report.breaches["asset_class_limits"]
    assert "currencies" in asset_breach
    assert "equities" not in asset_breach
    fx_payload = sector_breach["macro_fx"]
    assert fx_payload["exposure"] > fx_payload["limit"]
    macro_positions = {ex.symbol: ex for ex in report.exposures}
    assert macro_positions["EURUSD"].sector == "macro_fx"
    assert macro_positions["AAPL"].asset_class == "equities"
