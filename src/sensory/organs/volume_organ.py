...
class VolumeOrgan(SensoryOrgan):
    ...
    def _calculate_volume_trend_signal(self) -> VolumeSignal:
        ...
        confidence = min(abs(volume_trend_strength) * 5, 1.0)
    ...
    def _calculate_volume_momentum_signal(self) -> VolumeSignal:
        ...
        confidence = min(abs(vol_roc) * 2, 1.0)
    ...
    def _calculate_obv_signal(self) -> VolumeSignal:
        ...
        if len(volumes) > 0:
            ...
            normalized_obv = np.tanh(obv / (avg_volume * 10))  # Scale appropriately
        else:
            normalized_obv = 0.0
    ...
    def _calculate_vwap_signal(self, data: MarketData) -> VolumeSignal:
        ...
        # Calculate VWAP
        volume_price_sum = np.sum(volumes * typical_prices)
        ...
        # Normalize to [-1, 1] range
        normalized_signal = np.tanh(vwap_signal * 10)
