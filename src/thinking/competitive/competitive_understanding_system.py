"""
Competitive Understanding System
...
class CompetitiveUnderstandingSystem:
    """
    Identify and counter competing algorithmic traders.
    ...
    async def get_intelligence_stats(self) -> dict[str, object]:
        ...
        logger.warning(
            "get_intelligence_stats() is deprecated; use get_understanding_stats()",
        )
        stats = await self.get_understanding_stats()
        return self._with_legacy_aliases(stats)


class CompetitiveIntelligenceSystem(CompetitiveUnderstandingSystem):
    """Deprecated shim preserved for backwards compatibility.

    Instantiating this class emits a :class:`DeprecationWarning` directing
    callers to the ``CompetitiveUnderstandingSystem``.
    """

    def __init__(self, *args: Any, **kwargs: Any) -> None:
        warnings.warn(
            (
                "CompetitiveIntelligenceSystem is deprecated; "
