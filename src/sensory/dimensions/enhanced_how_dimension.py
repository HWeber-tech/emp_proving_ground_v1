...
class ICTPatternDetector:
    ...
    def _update_adaptive_parameters(self) -> None:
        ...
        self.volume_threshold_multiplier = (
            1.5 + (volume_std / avg_volume) if avg_volume > 0 else 1.5
        )
    ...
    def _detect_order_blocks(self) -> None:
        ...
        for i in range(5, len(price_changes) - 5):
            ...
            if (
                ...
                and volume > volume_avg * self.volume_threshold_multiplier
            ):
                ...
                volume_strength = volume / (
                    volume_avg * self.volume_threshold_multiplier
                )
                ...
                if self._is_new_order_block(order_block):
                    ...
                    self.order_blocks = [
                        ob for ob in self.order_blocks if ob.timestamp > cutoff_time
                    ]
    ...
    def _detect_fair_value_gaps(self) -> None:
        ...
        self.fair_value_gaps = [
            fvg for fvg in self.fair_value_gaps if fvg.timestamp > cutoff_time
        ]
    ...
    def _detect_liquidity_sweeps(self) -> None:
        ...
        swing_highs = find_peaks(
            prices, distance=5, prominence=self.displacement_threshold
        )[0]
        swing_lows = find_peaks(
            -prices, distance=5, prominence=self.displacement_threshold
        )[0]
        ...
        for high_idx in swing_highs[-5:]:  # Last 5 swing highs
            ...
            if current_price > swing_high and current_volume > avg_volume * 1.2:

                # Look for reversal after sweep
                reversal_strength = self._calculate_reversal_strength(
                    len(prices) - 1, "bearish"
                )
        ...
        for low_idx in swing_lows[-5:]:  # Last 5 swing lows
            ...
            if current_price < swing_low and current_volume > avg_volume * 1.2:

                reversal_strength = self._calculate_reversal_strength(
                    len(prices) - 1, "bullish"
                )
        ...
        self.liquidity_sweeps = [
            sweep for sweep in self.liquidity_sweeps if sweep.timestamp > cutoff_time
        ]
        ...
        if recent_structure != previous_structure:
            ...
            if confidence > 0.6:
                ...
                self.structure_shifts = [
                    shift
                    for shift in self.structure_shifts
    ...
    def _is_new_order_block(self, new_ob: OrderBlock) -> bool:
        ...
        for existing_ob in self.order_blocks:
            if (
                abs(new_ob.price_high - existing_ob.price_high)
                < self.displacement_threshold * 0.5
                and abs(new_ob.price_low - existing_ob.price_low)
                < self.displacement_threshold * 0.5
            ):
                return False
        return True
    ...
    def _is_new_fvg(self, new_fvg: FairValueGap) -> bool:
        ...
        for existing_fvg in self.fair_value_gaps:
            if (
                abs(new_fvg.gap_high - existing_fvg.gap_high)
                < self.displacement_threshold * 0.3
                and abs(new_fvg.gap_low - existing_fvg.gap_low)
                < self.displacement_threshold * 0.3
            ):
                return False
        return True
    ...
    def _is_new_liquidity_sweep(self, new_sweep: LiquiditySweep) -> bool:
        ...
        for existing_sweep in self.liquidity_sweeps:
            if (
                abs(new_sweep.swept_level - existing_sweep.swept_level)
                < self.displacement_threshold * 0.5
                and new_sweep.direction == existing_sweep.direction
            ):
                return False
        return True
...
class InstitutionalMechanicsEngine(DimensionalSensor):
    ...
    async def analyze_institutional_mechanics(
        ...
        try:
            ...
            institutional_score = (
                self.institutional_analyzer.get_institutional_footprint_score()
            )
