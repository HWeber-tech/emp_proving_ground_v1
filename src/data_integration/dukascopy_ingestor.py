...
class DukascopyIngestor:
    ...
    def download_tick_data(
        ...
        try:
            ...
            while current_date <= end_date:
                try:
                    ...
                    if day_data is not None and not day_data.empty:
                        ...
                        logger.info(
                            f"Downloaded {len(day_data)} ticks for {symbol} on {current_date}"
                        )
                    else:
            ...
            mask = (combined_data["timestamp"] >= start_time) & (
    ...
    def _download_day_data(self, symbol: str, date: date) -> Optional[pd.DataFrame]:
        """
        ...
        try:
            ...
            if response.status_code != 200:
                logger.debug(
                    f"No data available for {symbol} on {date} (HTTP {response.status_code})"
                )
                return None
    ...
    def _parse_binary_data(self, binary_data: bytes, date: date) -> Optional[pd.DataFrame]:
        """
        ...
        try:
            ...
            for i in range(num_records):
                ...
                # Unpack binary data
                timestamp_ms, bid, ask, bid_volume, ask_volume = struct.unpack(
                    ">Iffff", record_data
                )
