...
class DataPipeline:
    ...
    def calculate_technical_indicators(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        for period in [5, 10, 20, 50]:
            ...
            features_df[f"ema_{period}"] = (
                features_df["close"].ewm(span=period, adjust=False).mean()
            )
        ...
        features_df["bb_position"] = (features_df["close"] - features_df["bb_lower"]) / features_df[
            "bb_width"
        ]
    ...
    def create_target_variable(
        self, df: pd.DataFrame, threshold_pips: float = 10.0, forecast_horizon: int = 4
    ) -> pd.DataFrame:
...
def main():
    ...
    parser.add_argument(
        "--threshold-pips", type=float, default=10.0, help="Threshold in pips for target creation"
    )
    parser.add_argument(
        "--forecast-horizon", type=int, default=4, help="Number of bars to forecast ahead"
    )
    ...
    pipeline.run_pipeline(
        threshold_pips=args.threshold_pips, forecast_horizon=args.forecast_horizon
    )
