...
- [ ] `[core]` Live market data ingest configured (API keys, symbols, session calendars). _Progress: Institutional ingest builder now caches ReferenceDataLoader instrument inventory alongside API key posture, FX session calendars, macro calendars, and symbol metadata (contract size, pip decimals, margin currency, swap timing) for the configured symbol list, and now constrains the exported inventory to only configured symbols so manifests omit unsubscribed pairs, while managed ingest connectors CLI surfaces both inventory and metadata snapshots with swap rate context so operators receive margin-aware manifests; regression coverage confirms metadata exports for the London FX calendar, EURUSD-only inventory, and CLI wiring while live wiring remains outstanding.【F:src/data_foundation/ingest/configuration.py†L180-L220】【F:src/data_foundation/ingest/configuration.py†L955-L976】【F:tests/data_foundation/test_timescale_config.py†L320-L327】【F:tools/operations/managed_ingest_connectors.py†L420-L473】【F:tests/tools/test_managed_ingest_connectors.py†L60-L66】_
...
## Automation updates — 2025-10-19T22:56:36Z

### Last 4 commits
- 545134ba test(artifacts): add 5 files (2025-10-19)
- 9dc88555 feat(artifacts): add 5 files (2025-10-19)
- 3b2826e4 feat(artifacts): add 26 files (2025-10-19)
- 9b9fbc9c docs(docs): tune 1 file (2025-10-19)
