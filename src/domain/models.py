...
class CurrencyConverter:
    ...
    def get_rate(self, from_currency: str, to_currency: str) -> Decimal:
        ...
        if to_curr in self.rates and from_curr in self.rates[to_curr]:
            return Decimal('1.0') / Decimal(str(self.rates[to_curr][from_curr]))
        
        # Cross rate through USD
        if from_curr in self.rates and 'USD' in self.rates[from_curr]:
            usd_from = Decimal(str(self.rates[from_curr]['USD']))
            if to_curr in self.rates and 'USD' in self.rates[to_curr]:
                usd_to = Decimal(str(self.rates[to_curr]['USD']))
                return usd_from / usd_to
        
        logger.warning(f"Exchange rate not found: {from_curr} to {to_curr}")
        return Decimal('1.0')  # Default to 1:1
    
    def calculate_pip_value(self, instrument: Instrument, account_currency: str) -> Decimal:
        """Calculate pip value in account currency"""
        # For major pairs, pip value is typically $10 per 100k lot
        # This is a simplified calculation
        if instrument.margin_currency == account_currency:
            # Direct calculation
            pip_value = Decimal('10.0')  # $10 per pip for 100k lot
        else:
