    ...
    def calculate_contextual_weights(
        ...
        for dim, state in dimensional_states.items():
            if dim in weights:
                performance_multiplier = (
                    0.5 + state.performance_score
                )  # 0.5 to 1.5 range
                reliability_multiplier = 0.7 + (
                    state.reliability_score * 0.6
                )  # 0.7 to 1.3 range
        ...
        for dim, state in dimensional_states.items():
            if dim in weights and not state.is_healthy:
                # Significantly reduce weight for unhealthy engines
                weights[dim] *= 0.3
...
class GracefulDegradationManager:
    ...
    def get_degradation_strategy(
        self, dimensional_states: Dict[str, DimensionalState]
    ) -> Dict[str, Any]:
        """
        ...
        strategy = {
            ...
            "degraded_engines": [
                dim for dim in dimensional_states.keys() if dim not in healthy_engines
            ],
            "can_operate": len(healthy_engines) >= self.min_healthy_engines,
            "confidence_penalty": max(
                0.0, 1.0 - len(healthy_engines) / len(dimensional_states)
            ),
...
class NarrativeGenerator:
    ...
    def generate_narrative(
        ...
        direction = (
            ...
            else "bearish" if signal_strength < 0 else "neutral"
        )
        ...
        templates = self.narrative_templates.get(
            regime, ["Market showing {direction} bias"]
        )
        ...
        narrative = template.format(
            ...
            anomalies=self._get_anomalies_description(
                dimensional_readings.get("ANOMALY")
            ),
        )
    ...
    def _get_timing_description(
        self, when_reading: Optional[DimensionalReading]
    ) -> str:
    ...
    def _get_concerns_description(
        self, dimensional_readings: Dict[str, DimensionalReading]
    ) -> str:
    ...
    def _get_factors_description(
        self, dimensional_readings: Dict[str, DimensionalReading]
    ) -> str:
        """Get factors description."""
        return "multiple"  # Simplified

    def _get_catalyst_description(
        self, dimensional_readings: Dict[str, DimensionalReading]
    ) -> str:
    ...
    def _get_anomalies_description(
        self, anomaly_reading: Optional[DimensionalReading]
    ) -> str:
...
class MasterOrchestrator:
    ...
    async def update(
        ...
        try:
            ...
            dimensional_readings = await self._update_dimensional_engines(
            ...
            degradation_strategy = self.degradation_manager.get_degradation_strategy(
                self.dimensional_states
            )

            # Check if we can operate
            if not degradation_strategy["can_operate"]:
                return self._create_degraded_synthesis(
                    market_data.timestamp, degradation_strategy
                )
            ...
            logger.debug(
                f"Synthesis complete: signal={synthesis_result.signal_strength:.3f}, "
    ...
    async def _update_dimensional_engines(
        self, market_data: MarketData, order_book: Optional[OrderBookSnapshot]
        ...
        for dim, task in tasks:
            try:
                ...
                results[dim] = self._create_error_reading(
                    dim, market_data.timestamp, str(e)
                )

        return results

    def _update_dimensional_states(
        self, dimensional_readings: Dict[str, DimensionalReading]
    ) -> None:
        ...
        for dim, reading in dimensional_readings.items():
            ...
            if reading.confidence > 0 and reading.data_quality > 0:
                ...
                state.performance_score = (
                    state.performance_score * 0.9 + performance * 0.1
                )

            # Update reliability score (based on error rate)
            # Assume 10 successful updates
            error_rate = state.error_count / max(1, state.error_count + 10)
            state.reliability_score = 1.0 - error_rate

            # Update health status
            state.is_healthy = self.degradation_manager.assess_engine_health(state)

    def _determine_consensus_regime(
        self, dimensional_readings: Dict[str, DimensionalReading]
    ) -> MarketRegime:
    ...
    async def _synthesize_dimensional_intelligence(
        ...
        synthesis_mode = self._determine_synthesis_mode(
            dimensional_readings, consensus_level
        )
        ...
        # Extract evidence
        evidence = self._extract_synthesis_evidence(
            dimensional_readings, contextual_weights
        )

        # Generate warnings
        warnings = self._generate_synthesis_warnings(
            dimensional_readings, degradation_strategy
        )
    ...
    def _extract_synthesis_evidence(
        ...
        # Synthesis evidence
        evidence["consensus_level"] = self._calculate_consensus_level(
            dimensional_readings
        )
        evidence["healthy_engines"] = sum(
            1 for state in self.dimensional_states.values() if state.is_healthy
        )
    ...
    def _generate_synthesis_warnings(
        ...
        for dim, reading in dimensional_readings.items():
            if reading.warnings:
                # Limit to 2 warnings per engine
    ...
    def _create_degraded_synthesis(
        self, timestamp: datetime, degradation_strategy: Dict[str, Any]
    ) -> SynthesisResult:
    ...
    def _create_error_synthesis(
        self, timestamp: datetime, error_msg: str
    ) -> SynthesisResult:
    ...
    def _create_error_reading(
        self, dimension: str, timestamp: datetime, error_msg: str
    ) -> DimensionalReading:
    ...
    def get_system_health(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "average_performance": np.mean(
                [state.performance_score for state in self.dimensional_states.values()]
            ),
            "average_reliability": np.mean(
                [state.reliability_score for state in self.dimensional_states.values()]
            ),
