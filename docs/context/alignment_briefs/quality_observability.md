...
- Add regression tickets/tests for uncovered modules (`operational.metrics`,
  ...
  - Progress: Kibana dashboard deployment CLI zips Saved Objects exports, supports API key or basic auth flows, prints human-readable saved-object summaries, and allows partial failure handling so operations can refresh observability panels without manual cURL scripts, with pytest coverage validating both success and failure paths.【F:tools/observability/deploy_kibana_dashboards.py†L1-L278】【F:tests/tools/test_deploy_kibana_dashboards.py†L1-L110】
  - Progress: Local Elasticsearch deployment helper wraps Docker Compose bring-up,
    waits on cluster health, and ingests verification logs while shipping a
    doc-packed guide so operators can stage observability stacks with one
    command, including built-in compose resolution, health polling, and ingest
    verification flows.【F:tools/observability/deploy_elasticsearch_cluster.py†L1-L307】【F:docker/elasticsearch/README.md†L1-L23】【F:docs/observability/elasticsearch_cluster.md†L1-L64】
