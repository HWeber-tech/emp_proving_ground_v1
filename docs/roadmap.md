...
- [ ] `[obs]` Attribution coverage â‰¥ **90%** of orders have belief + probes; no Ïƒ explosions (bounded norms). _Progress: AlphaTradeLoopRunner now threads diary coverage into trade metadata, intents, and trade outcomes while only attaching attribution when diaries exist and pruning stale payloads, and TradingManager propagates coverage snapshots across execution, fallback, and risk rejection metadata so guardrail events stay aligned; pytest drives executed runs plus missing-diary gaps to assert coverage snapshots persist and warnings fire.【F:src/orchestration/alpha_trade_runner.py†L303-L347】【F:src/orchestration/alpha_trade_runner.py†L634-L664】【F:tests/orchestration/test_alpha_trade_runner.py†L261-L316】【F:tests/orchestration/test_alpha_trade_runner.py†L966-L994】【F:src/trading/trading_manager.py†L1542-L1645】【F:src/trading/trading_manager.py†L3870-L3892】【F:tests/trading/test_trading_manager_execution.py†L1092-L1103】_
