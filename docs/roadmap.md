...
- [x] `[adapt]` **LinearAttentionRouter** (flagâ€‘guarded) for policy arbitration. _Progress: PolicyRouter now instantiates a LinearAttentionRouter when the flag is enabled, seeds it with replayed reflection summaries, reorders ranked tactics with novelty telemetry, and persists before/after history while ingest and route tests cover the seeded novelty path.【F:src/thinking/adaptation/policy_router.py†L217-L260】【F:src/thinking/adaptation/policy_router.py†L675-L1372】【F:tests/thinking/test_policy_router.py†L288-L327】_
...
- [ ] `[obs]` Attribution coverage â‰¥ **90%** of orders have belief + probes; no Ïƒ explosions (bounded norms). _Progress: AlphaTradeLoopRunner now collapses rationale text into bounded `_build_brief_explanation` payloads, threads the trimmed attribution onto trade metadata, intents, outcomes, and loop metadata even when diaries are missing, and strips stale policy/diary/brief_explanation/attribution fields whenever an iteration lacks them so coverage snapshots stay aligned and honest across loop results.【F:src/orchestration/alpha_trade_runner.py†L266-L390】【F:src/orchestration/alpha_trade_runner.py†L301-L335】【F:src/orchestration/alpha_trade_runner.py†L451-L485】【F:src/orchestration/alpha_trade_runner.py†L946-L982】【F:src/orchestration/alpha_trade_runner.py†L1002-L1049】【F:src/orchestration/alpha_trade_runner.py†L1040-L1071】 Diaries from 2025-10-20 (e.g., the replay `diary-011424`, counterfactual mirror `counterfactual_diary-010924`, and the new bootstrap ledger drop `decision_diary-020120`) show the recorded `brief_explanation`, guardrail probes, and coverage snapshots while keeping metadata empty when no explanation survives.【F:artifacts/diaries/2025/10/20/diary-011424/diary.json†L474】【F:artifacts/diaries/2025/10/20/diary-011424/diary.json†L687】【F:artifacts/diaries/2025/10/20/counterfactual_diary-010924/counterfactual_diary.json†L412】【F:artifacts/diaries/2025/10/20/counterfactual_diary-010924/counterfactual_diary.json†L635】【F:artifacts/diaries/2025/10/20/decision_diary-020120/decision_diary.json†L4-L20】 Fresh diaries from the same run (`diary-022138`, `diary-022639`, and counterfactual twin `counterfactual_diary-022138`) keep `brief_explanation` coverage and guardrail metadata intact across live, paper, and counterfactual executions while still blanking fields when attribution is absent.【F:artifacts/diaries/2025/10/20/diary-022138/diary.json†L512】【F:artifacts/diaries/2025/10/20/diary-022138/diary.json†L2079】【F:artifacts/diaries/2025/10/20/diary-022639/diary.json†L474】【F:artifacts/diaries/2025/10/20/counterfactual_diary-022138/counterfactual_diary.json†L412】 Latest diary export at 03:10 (`diary-031003`) continues to log the `brief_explanation`, guardrail probes, and drift decisions while keeping blocked dimensions empty so the newest evidence pack stays in parity with governance coverage.【F:artifacts/diaries/2025/10/20/diary-031003/diary.json†L474】【F:artifacts/diaries/2025/10/20/diary-031003/diary.json†L480】【F:artifacts/diaries/2025/10/20/diary-031003/diary.json†L504】【F:artifacts/diaries/2025/10/20/diary-031003/diary.json†L799】 TradingManager normalises `brief_explanation` first, rehydrates attribution from trade outcomes when intents omit it, trims blank policy/diary identifiers before counting coverage, and keeps coverage counters warned when trimmed payloads are missing.【F:src/trading/trading_manager.py†L3947】【F:src/trading/trading_manager.py†L3953】【F:src/trading/trading_manager.py†L3956-L3966】【F:src/trading/trading_manager.py†L4160】【F:src/trading/trading_manager.py†L4166】【F:src/trading/trading_manager.py†L4180】 Orchestration and trading manager tests now lock the executed, fallback, rejection, failure, and missing-diary paths so attribution coverage stays at 100% in the exercised scenarios.【F:tests/orchestration/test_alpha_trade_runner.py†L484-L522】【F:tests/orchestration/test_alpha_trade_runner.py†L1314-L1357】【F:tests/trading/test_trading_manager_execution.py†L1102】【F:tests/trading/test_trading_manager_execution.py†L1163】【F:tests/trading/test_trading_manager_execution.py†L1338】_
...
- [x] `[risk]` **0** invariant violations during exploration; freeze triggers on violations/drift immediately. _Progress: AlphaTradeLoopRunner now promotes risk guardrail incidents into critical exploration freeze triggers with deduplicated invariant violations and guardrail metadata, while TradingManager refuses attribution samples lacking belief or probe identifiers so guardrail evidence stays audit-ready; regression coverage proves synthetic invariant breaches immediately freeze exploration with guardrail triggers.【F:src/orchestration/alpha_trade_runner.py†L832-L1360】【F:src/trading/trading_manager.py†L4042-L4093】【F:tests/orchestration/test_alpha_trade_runner.py†L728-L771】【F:tests/trading/test_trading_manager_execution.py†L1179-L1229】_
- [x] `[obs]` Evolution KPIs live: timeâ€‘toâ€‘candidate, promotion rate, budget usage, rollback latency. _Progress: `evaluate_evolution_kpis` fuses experimentation turnaround, ledger promotions, exploration budget, and rollback latency into Prometheus-backed gauges that feed the observability dashboard’s Evolution KPIs panel with regression coverage locking both the KPI snapshot and rendered panel wiring, and fresh regression now locks missing `completed_at` normalisation before metrics publish.【F:src/operations/evolution_kpis.py†L1-L740】【F:src/operational/metrics.py†L182-L296】【F:src/operations/observability_dashboard.py†L569-L720】【F:tests/operations/test_evolution_kpis.py†L1-L120】【F:tests/operations/test_evolution_kpis.py†L159-L188】【F:tests/operations/test_observability_dashboard.py†L399-L527】_
...
- [ ] `[core]` Live market data ingest configured (API keys, symbols, session calendars). _Progress: Institutional ingest builder now caches ReferenceDataLoader instrument inventory alongside API key posture, FX session calendars, macro calendars, and symbol metadata (contract size, pip decimals, margin currency, swap timing, long/short swap rates) for the configured symbol list, short-circuits to an empty inventory when no symbols are configured so unprovisioned environments never leak the full catalog, and still constrains the exported inventory to only configured symbols so manifests omit unsubscribed pairs, while managed ingest connectors CLI surfaces both inventory and metadata snapshots with swap rate context using a sanitised symbol snapshot so operators receive margin-aware manifests; regression coverage now locks EURUSD long/short swap rates across the ingest config and managed connectors CLI while live wiring remains outstanding.【F:src/data_foundation/ingest/configuration.py†L199-L220】【F:src/data_foundation/ingest/configuration.py†L955-L976】【F:src/data_foundation/ingest/institutional_vertical.py†L81-L103】【F:src/data_foundation/ingest/institutional_vertical.py†L904-L953】【F:tests/data_foundation/test_timescale_config.py†L320-L340】【F:tools/operations/managed_ingest_connectors.py†L404-L469】【F:tests/tools/test_managed_ingest_connectors.py†L95-L118】_
...
- [ ] **Timeâ€‘toâ€‘candidate** â‰¤ 24h (idea â†’ scored in replay). _Progress: Findings memory now persists a `completed_at` column, backfills it during promotions, and the SLA analytics prefer `completed_at` before falling back to progress/tested timestamps so breach reports stay anchored to the true completion time while the metrics CLI keeps returning non-zero exit codes when breaches persist.【F:emp/core/findings_memory.py†L12-L101】【F:emp/core/findings_memory.py†L236-L441】【F:emp/cli/emp_cycle_metrics.py†L1-L70】 The scheduler CLI quick-screens ideas, applies UCB-lite promotion with bounded `--max-quick`/`--max-full` budgets, and stamps explicit `evidence:quick-screen`/`evidence:full-eval` alongside `coverage:regression` notes so candidates enter replay within the 24 h envelope, adds the same evidence markers when full evaluations timeout or error so aborted runs still record SLA artefacts, and Evolution KPI snapshots normalise partially evaluated candidates by filling `completed_at` gaps for honest SLA reporting even while replay scoring is pending under regression coverage, including fallback to the latest progress timestamp when `completed_at` is missing.【F:emp/cli/emp_cycle_scheduler.py†L32-L371】【F:emp/cli/emp_cycle_scheduler.py†L351-L365】【F:src/operations/evolution_kpis.py†L220-L275】【F:tests/operations/test_evolution_kpis.py†L96-L139】【F:tests/operations/test_evolution_kpis.py†L159-L188】【F:tests/emp_cycle/test_time_to_candidate.py†L1-L139】【F:tests/emp_cycle/test_cycle_scheduler.py†L1-L86】
- [x] **Promotion integrity**: 100% promoted strategies have ledger artifacts & pass regimeâ€‘grid gates. _Progress: Strategy registry now invokes the promotion guard before registering champions, surfaces guard failures as StrategyRegistryError, and regression coverage proves approvals succeed only after required regimes are recorded in the ledger and diary stores.【F:src/governance/strategy_registry.py†L348-L367】【F:tests/governance/test_strategy_registry.py†L271-L405】 Latest ledger exports for `alpha.live` record the risk approval, paper staging, and evidence linkage demanded by the guardrail.【F:artifacts/ledger_exports/2025/10/20/ledger/ledger.json†L1-L26】
...
- [ ] **Attribution coverage** â‰¥ 90% (orders with belief + probes + brief explanation). _Progress: The loop runner now feeds the trimmed `_build_brief_explanation` output into trade metadata, intents, outcomes, and loop metadata regardless of diary availability, and strips stale policy/diary/brief_explanation/attribution fields whenever they are absent so attribution and coverage snapshots stay aligned without carry-over noise.【F:src/orchestration/alpha_trade_runner.py†L266-L390】【F:src/orchestration/alpha_trade_runner.py†L301-L335】【F:src/orchestration/alpha_trade_runner.py†L451-L485】【F:src/orchestration/alpha_trade_runner.py†L946-L982】【F:src/orchestration/alpha_trade_runner.py†L1002-L1049】【F:src/orchestration/alpha_trade_runner.py†L1040-L1071】 Diaries from 2025-10-20 (replay `diary-011424`, counterfactual mirror `counterfactual_diary-010924`, replay `diary-015240`, and the new bootstrap ledger drop `decision_diary-020120`) capture the updated attribution block with brief explanations, guardrail probes, and empty metadata when explanations are missing while logging 100% diary coverage under forced-paper guardrails.【F:artifacts/diaries/2025/10/20/diary-011424/diary.json†L474】【F:artifacts/diaries/2025/10/20/diary-011424/diary.json†L687】【F:artifacts/diaries/2025/10/20/counterfactual_diary-010924/counterfactual_diary.json†L412】【F:artifacts/diaries/2025/10/20/counterfactual_diary-010924/counterfactual_diary.json†L635】【F:artifacts/diaries/2025/10/20/diary-015240/diary.json†L474-L507】【F:artifacts/diaries/2025/10/20/decision_diary-020120/decision_diary.json†L4-L20】 TradingManager now prefers `brief_explanation`, rebuilds attribution from trade outcomes when intents lack it, trims blank policy/diary identifiers before counting coverage, and enforces coverage counters off the trimmed payloads to keep warning thresholds meaningful.【F:src/trading/trading_manager.py†L3947】【F:src/trading/trading_manager.py†L3953】【F:src/trading/trading_manager.py†L3956-L3966】【F:src/trading/trading_manager.py†L4160】【F:src/trading/trading_manager.py†L4166】【F:src/trading/trading_manager.py†L4180】 Regression coverage exercises execution success, failure, rejection, and missing-diary paths so attribution coverage remains intact across scenarios.【F:tests/orchestration/test_alpha_trade_runner.py†L484-L522】【F:tests/orchestration/test_alpha_trade_runner.py†L1314-L1357】【F:tests/trading/test_trading_manager_execution.py†L1102】【F:tests/trading/test_trading_manager_execution.py†L1163】【F:tests/trading/test_trading_manager_execution.py†L1338】
...
- [x] `make run-paper` â€” paper 24/7 profile with dashboards. _Progress: Makefile routes to the runtime CLI `paper-run` subcommand which boots the guardian, enforces minimum runtime thresholds, streams progress, and persists structured summaries for dashboards while the guardian now fails runs and surfaces stop reasons when memory growth thresholds trigger, with the CLI printing threshold metadata, PASS/FAIL objective compliance, and honouring a `--minimum-runtime-hours` override that defaults to seven days under pytest coverage.【F:Makefile†L98-L100】【F:src/runtime/cli.py†L204-L493】【F:src/runtime/paper_run_guardian.py†L250-L462】【F:tests/runtime/test_paper_run_guardian.py†L62-L217】_
...
## Automation updates — 2025-10-20T03:10:31Z

### Last 4 commits
- 5b3a697b test(artifacts): add 3 files (2025-10-20)
- cda1fd85 refactor(thinking): tune 3 files (2025-10-20)
- aabec9c0 test(.telemetry): tune 1 file (2025-10-20)
- 18fb21cb docs(docs): tune 1 file (2025-10-20)
...
### Last 4 commits
- 13e4a13c test(artifacts): add 43 files (2025-10-20)
- 18bf245c test(artifacts): add 14 files (2025-10-20)
- f00f3102 test(artifacts): tune 9 files (2025-10-20)
- 56558c92 docs(docs): tune 1 file (2025-10-20)

## Automation updates — 2025-10-20T02:27:03Z

### Last 4 commits
- cab0077d test(artifacts): add 15 files (2025-10-20)
- abfb0092 test(artifacts): tune 9 files (2025-10-20)
- e4c21d7c test(artifacts): add 19 files (2025-10-20)
- 6ff427ee test(emp): tune 4 files (2025-10-20)

## Automation updates — 2025-10-20T02:13:38Z

### Last 4 commits
- 784df88a test(emp): tune 2 files (2025-10-20)
- 3c640006 chore(emp): tune 1 file (2025-10-20)
- 71928641 test(artifacts): add 14 files (2025-10-20)
- 496b8119 docs(docs): tune 1 file (2025-10-20)
...
### Last 4 commits
- 51f8bef7 test(artifacts): add 8 files (2025-10-20)
- b8c84718 test(artifacts): add 16 files (2025-10-20)
- bd3e9b5e test(artifacts): add 20 files (2025-10-20)
- ed3d7715 docs(docs): tune 1 file (2025-10-20)

## Automation updates — 2025-10-20T01:03:38Z

### Last 4 commits
- 4f055c69 test(artifacts): add 15 files (2025-10-20)
- ba573f47 docs(docs): tune 1 file (2025-10-20)
...
### Last 4 commits
- 3118fc88 refactor(orchestration): tune 4 files (2025-10-20)
- 0d44a46d test(emp): tune 2 files (2025-10-20)
- 877f5ed5 feat(_automation): add 2 files (2025-10-20)
- e0696d6f docs(docs): tune 1 file (2025-10-20)

## Automation updates — 2025-10-20T00:18:22Z

### Last 4 commits
- 9381451c refactor(runtime): tune 1 file (2025-10-20)
- 51a774c7 test(.telemetry): tune 2 files (2025-10-20)
- c44ec707 docs(docs): tune 1 file (2025-10-20)
- 5d0a5cbc refactor(trading): tune 2 files (2025-10-20)

## Automation updates — 2025-10-19T23:57:54Z

### Last 4 commits
- dce22ea1 test(artifacts): add 6 files (2025-10-20)
- cde5831a test(.telemetry): tune 1 file (2025-10-20)
- 7093bd73 docs(docs): tune 1 file (2025-10-20)
- 0492382f refactor(data_foundation): tune 2 files (2025-10-20)

## Automation updates — 2025-10-19T23:55:49Z

### Last 4 commits
- 0492382f refactor(data_foundation): tune 2 files (2025-10-20)
- 97298ca8 refactor(trading): tune 2 files (2025-10-20)
- 3eab155a refactor(runtime): tune 3 files (2025-10-20)
- f763d9a5 test(artifacts): add 21 files (2025-10-20)

## Automation updates — 2025-10-19T23:41:40Z

### Last 4 commits
- fa4cab83 test(.telemetry): tune 3 files (2025-10-20)
- 35160c19 test(.telemetry): tune 1 file (2025-10-20)
- b73360bb refactor(orchestration): tune 4 files (2025-10-20)
- ff8f7b6d docs(docs): tune 1 file (2025-10-20)

## Automation updates — 2025-10-19T23:29:05Z

### Last 4 commits
- 131ad65b feat(artifacts): add 33 files (2025-10-20)
- 42a54ec9 feat(artifacts): add 5 files (2025-10-20)
- 54e9a43e refactor(orchestration): tune 2 files (2025-10-20)
- a9271374 docs(docs): tune 1 file (2025-10-20)

## Automation updates — 2025-10-19T23:10:12Z

### Last 4 commits
- fcfb1dfb test(emp): tune 3 files (2025-10-19)
- 1af42424 refactor(data_foundation): tune 2 files (2025-10-19)
- 4e8dbd82 docs(docs): tune 1 file (2025-10-19)
- 545134ba test(artifacts): add 5 files (2025-10-19)

## Automation updates — 2025-10-19T22:13:26Z

### Last 4 commits
- 04599605 refactor(thinking): tune 2 files (2025-10-20)
- 6c560d44 test(artifacts): add 7 files (2025-10-20)
- a737b6e2 feat(_automation): add 2 files (2025-10-20)
- 5364f8d7 docs(docs): tune 1 file (2025-10-20)

## Automation updates — 2025-10-19T22:02:17Z

### Last 4 commits
- 7a3a0d9f test(artifacts): add 20 files (2025-10-20)
- 4800781d test(.telemetry): tune 3 files (2025-10-20)
- b0ad6ccb test(.telemetry): tune 1 file (2025-10-20)
- 454652de docs(docs): tune 1 file (2025-10-19)

## Automation updates — 2025-10-19T21:52:06Z

### Last 4 commits
- 5f23eb19 refactor(runtime): tune 3 files (2025-10-19)
- 4ca482d5 test(.telemetry): tune 1 file (2025-10-19)
- 1da467f3 refactor(runtime): tune 1 file (2025-10-19)
- fad6cc47 docs(docs): tune 1 file (2025-10-19)

## Automation updates — 2025-10-19T21:48:01Z

### Last 4 commits
- 88849b07 test(artifacts): add 7 files (2025-10-19)
- b0bdea79 refactor(operations): tune 2 files (2025-10-19)
- e69b1443 docs(docs): tune 1 file (2025-10-19)
- e69ba747 feat(artifacts): add 8 files (2025-10-19)

## Automation updates — 2025-10-19T21:20:14Z

### Last 4 commits
- 55c19789 feat(artifacts): add 21 files (2025-10-19)
- 998c676a test(artifacts): add 20 files (2025-10-19)
- d6bca11b docs(docs): tune 1 file (2025-10-19)
- 5a3a3042 feat(_automation): add 2 files (2025-10-19)
...
## Automation updates — 2025-10-20T00:26:15Z

### Last 4 commits
- 2b19e302 test(artifacts): add 5 files (2025-10-20)
- 34bb5a57 test(artifacts): add 3 files (2025-10-20)
...
## Automation updates — 2025-10-20T01:55:39Z

### Last 4 commits
- f4f8fbf9 refactor(emp): tune 3 files (2025-10-20)
- 66a42a7e test(artifacts): add 3 files (2025-10-20)
- a4a22f7c test(.telemetry): tune 1 file (2025-10-20)
- 9a66ad77 docs(docs): tune 1 file (2025-10-20)
