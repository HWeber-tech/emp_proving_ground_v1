...
class TestContextualFusion:
    ...
    async def test_fusion_with_multiple_data_points(self, fusion_engine, data_generator):
        ...
        assert confidences[-1] >= confidences[0] - 0.1, "Confidence should not degrade significantly"
    ...
    async def test_narrative_generation(self, fusion_engine, data_generator):
        ...
        for narrative in narratives:
            ...
            market_terms = ['market', 'price', 'trend', 'technical', 'fundamental', 'institutional']
...
class TestSystemIntegration:
    ...
    async def test_performance_characteristics(self, fusion_engine, data_generator):
        ...
        assert avg_time_per_analysis < 1.0, f"Analysis taking too long: {avg_time_per_analysis:.3f}s per analysis"
...
class TestScenarioValidation:
    ...
    async def test_ranging_market_detection(self, fusion_engine, data_generator):
        ...
        assert avg_abs_score < 0.4, f"Should detect ranging market, got average absolute score: {avg_abs_score:.3f}"
    ...
    async def test_volatility_detection(self, fusion_engine, data_generator):
        ...
        volatility_detected = (
            any('volatil' in factor.lower() for factor in final_synthesis.risk_factors) or
            any('stress' in factor.lower() for factor in final_synthesis.risk_factors) or
            'VOLATILE' in final_synthesis.dominant_narrative.name
        )
