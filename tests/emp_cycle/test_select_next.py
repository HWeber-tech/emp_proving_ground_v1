...
def test_pick_next_prefers_ucb_score(tmp_path):
    ...
    params = {"alpha": 1}
    fid1 = findings_memory.add_idea(conn, params, novelty=0.9).id
    fid2 = findings_memory.add_idea(conn, {"alpha": 2}, novelty=0.3).id
    fid3 = findings_memory.add_idea(conn, {"alpha": 3}, novelty=0.5).id
