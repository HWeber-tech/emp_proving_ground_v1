...
- Risk analytics now ship a Monte Carlo simulation helper that generates geometric Brownian motion paths, surfaces VaR/ES metrics, and supports antithetic variates for deterministic reports.【F:src/risk/analytics/monte_carlo.py†L1-L171】【F:tests/risk/analytics/test_monte_carlo.py†L1-L96】
...
- Progress: Execution throughput snapshots now embed `throttle_active`, state, reasons, retry timers, remaining capacity, and scope metadata so compliance dashboards and evidence packs can show exactly why trades were blocked without cross-referencing throttle internals; throttled versus disabled flows are locked under regression coverage.【F:src/trading/trading_manager.py†L542-L906】【F:src/trading/trading_manager.py†L1433-L1510】【F:src/trading/trading_manager.py†L1891-L2054】【F:tests/trading/test_trading_manager_execution.py†L2046-L2170】
- Progress: Trade throttle now exposes notional quotas alongside trade-count limits, labelling `notional_limit` states and surfacing consumed/remaining notional through execution stats, throughput payloads, experiment events, throttle history, and the performance baseline CLI so compliance reviews inherit capital-usage evidence; regressions cover notional limit blocks, rollback refunds, and CLI wiring.【F:src/trading/execution/trade_throttle.py†L214-L367】【F:src/trading/trading_manager.py†L542-L906】【F:tests/trading/test_trade_throttle.py†L113-L187】【F:tests/trading/test_trading_manager_execution.py†L2180-L2267】【F:tools/performance_baseline.py†L23-L208】
...
- Progress: Stage-gate enforcement now forces paper routes for experiment and paper ledger stages, downgrades limited-live defaults to pilot when no ledger record exists, merges drift/audit reasons into routing metadata, and proves via orchestration and gate tests that strategies without governance records cannot execute live even when drift is nominal.【F:src/trading/execution/release_router.py†L80-L310】【F:src/governance/policy_ledger.py†L550-L623】【F:tests/trading/test_release_execution_router.py†L88-L167】【F:tests/orchestration/test_alpha_trade_loop.py†L24-L162】【F:tests/trading/test_drift_sentry_gate.py†L190-L212】
- Progress: Risk guardrail near-miss incidents now impose force-paper cooldowns, persist remaining time and reasons in execution stats, attach `risk_guardrail` blocks onto trade intent metadata, and drive release routing to keep orders on paper until the cooldown expires, with regression coverage proving the forced routes and telemetry snapshots.【F:src/trading/trading_manager.py†L546-L590】【F:src/trading/trading_manager.py†L2011-L2219】【F:src/trading/execution/release_router.py†L433-L547】【F:tests/trading/test_trading_manager_execution.py†L730-L872】
...
- Progress: Promotion tooling now builds `PolicyTraceability` metadata containing diary slices, git code hashes, and config-delta digests, wiring the payload into both single-policy and graduation CLIs so every ledger record carries reproducible provenance under regression coverage that inspects stored trace entries and CLI summaries.【F:src/governance/policy_traceability.py†L1-L230】【F:tools/governance/promote_policy.py†L323-L379】【F:tools/governance/alpha_trade_graduation.py†L168-L221】【F:tests/governance/test_policy_traceability.py†L1-L112】【F:tests/tools/test_promote_policy_cli.py†L60-L118】
...
- Progress: Evidence archiving now mirrors decision diaries, drift reports, ledger exports, and evolution KPIs into dated `artifacts/` folders via `archive_artifact`, with runtime pipelines and governance CLIs invoking the helper under regression coverage so compliance packets land automatically with run-scoped provenance.【F:src/artifacts/archive.py†L1-L95】【F:src/governance/policy_ledger.py†L394-L420】【F:src/understanding/decision_diary.py†L447-L465】【F:tools/operations/nightly_replay_job.py†L573-L590】【F:scripts/generate_evolution_lab.py†L171-L185】【F:tests/artifacts/test_archive.py†L1-L45】
...
- Progress: Trading manager now installs the PaperBrokerExecutionAdapter as the limited-live engine with stage defaults, risk-context snapshots, and broker failure capture so decision diaries persist the last error payload when simulations falter. Bootstrap can attach a REST `PaperTradingApiAdapter` when extras are supplied, exercising real HTTP round-trips, cleanup callbacks, and failure retries under pytest coverage so paper pilots inherit deterministic routing without bespoke wiring, and a dedicated simulation runner/CLI now drives the bootstrap runtime end-to-end to capture broker orders, errors, and diary counts for roadmap evidence packs; recent hardening timestamps first/last orders, records the most recent error, threads the risk snapshot into bootstrap summaries, and captures residual broker failures even when history buffers are empty while the adapters snapshot the latest broker submission request/response payloads for diaries and HTTP error evidence, with regression coverage asserting the cloned telemetry retains ISO timestamps, status codes, and risk context without mutation.【src/trading/execution/paper_broker_adapter.py:259】【src/trading/execution/paper_broker_adapter.py:457】【src/trading/integration/paper_trading_api.py:321】【src/trading/integration/paper_trading_api.py:503】【src/runtime/bootstrap_runtime.py:461】【src/runtime/paper_simulation.py:193】【tests/runtime/test_bootstrap_paper_broker.py:64】【tests/runtime/test_paper_trading_simulation_runner.py:139】【tests/trading/test_paper_broker_adapter.py:296】【tests/trading/test_paper_trading_api_adapter.py:69】【tests/integration/test_paper_trading_simulation.py:423】 Failover thresholds and cooldowns now propagate from runtime extras and the simulation CLI, blocking additional orders during cooldown windows while metrics surface the failover snapshot and guardrail coverage exercises the block/unblock cycle end to end.【F:src/trading/execution/paper_broker_adapter.py†L57-L379】【F:src/runtime/predator_app.py†L2571-L2649】【F:tools/trading/run_paper_trading_simulation.py†L123-L251】【F:tests/trading/test_paper_broker_adapter.py†L345-L379】 Latest revision fixes the submission guard so `_fetch_broker_submission` still returns broker metadata when placement raises, keeping both last-order and last-error diaries populated with request/response payloads under fresh async regression coverage.【F:src/trading/execution/paper_broker_adapter.py†L472-L540】【F:tests/trading/test_paper_broker_adapter.py†L324-L377】
  ...
  - Progress: Paper simulation loop now honours an external stop event and the CLI wraps SIGINT/SIGTERM with temporary handlers so drills can shut down gracefully without leaving background tasks, while integration guardrails replay persistent 500 responses to assert PaperTradingApiError telemetry, 0% success ratios, and diary error snapshots land in the evidence pack.【src/runtime/paper_simulation.py:83】【tools/trading/run_paper_trading_simulation.py:25】【tests/runtime/test_paper_trading_simulation_runner.py:289】【tests/integration/test_paper_trading_simulation.py:337】
  - Progress: Simulation loop now emits `PaperTradingSimulationProgress` snapshots and the CLI’s `--progress-interval` flag streams JSON updates so operators can monitor orders, failover posture, and paper metrics in real time while guardrail coverage verifies async and sync callbacks receive the enriched payloads.【F:src/runtime/paper_simulation.py†L119-L408】【F:tools/trading/run_paper_trading_simulation.py†L191-L315】【F:tests/runtime/test_paper_trading_simulation_runner.py†L217-L285】
...
- Progress: Execution pipeline now enforces configurable trade throttles via the trading manager, layering scope-aware windows, minimum spacing guards, and countdown metadata so bursty strategies respect governance cooldowns; snapshot payloads now expose `max_trades`, remaining trade credits, bounded utilisation, retry/reset timers, and the resolved scope/context dictionaries (plus `scope_key`) documented in operator guides, with regression coverage spanning throttle helpers, trading-manager snapshots, and docs. Fractional window durations now carry through reason strings and human-readable countdowns (`2.5 seconds` instead of truncating), keeping compliance dashboards precise when sub-second guardrails are configured.【F:src/trading/execution/trade_throttle.py†L204-L367】【F:src/trading/trading_manager.py†L542-L906】【F:docs/performance/trading_throughput_monitoring.md†L16-L96】【F:tests/trading/test_trade_throttle.py†L15-L160】【F:tests/trading/test_trading_manager_execution.py†L2046-L2170】
  ...
  - Progress: Backlog breaches now trigger external throttle cooldowns that surface lag metadata, operator messages, and retry timers through throttle snapshots, execution stats, and experiment events so compliance reviewers know why bursts halted and when capacity returns, with regression coverage across throttle and trading-manager guardrails.【F:src/trading/execution/trade_throttle.py†L382-L507】【F:src/trading/trading_manager.py†L1533-L1604】【F:tests/trading/test_trade_throttle.py†L372-L403】【F:tests/trading/test_trading_manager_execution.py†L2172-L2238】
