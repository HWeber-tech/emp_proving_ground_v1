...
def generate_test_tick_data(symbol: str, num_days: int = 7, base_price: float = 1.1000) -> pd.DataFrame:
    """
    ...
    bid_volumes = np.random.uniform(base_volume, base_volume * 10, len(time_range)) * session_multiplier
    ask_volumes = np.random.uniform(base_volume, base_volume * 10, len(time_range)) * session_multiplier
