...
class RiskConfig(BaseModel):
    ...
    max_leverage: Decimal = Field(
        default=Decimal("10.0"), description="Maximum allowed leverage"
    )
    ...
    max_drawdown_pct: Decimal = Field(
        default=Decimal("0.25"), description="Maximum drawdown before stopping"
    )
    min_position_size: int = Field(
        default=1000, description="Minimum position size in units"
    )
    max_position_size: int = Field(
        default=1000000, description="Maximum position size in units"
    )
    mandatory_stop_loss: bool = Field(
        default=True, description="Whether stop loss is mandatory"
    )
    research_mode: bool = Field(
        default=False, description="Research mode disables some safety checks"
    )
    ...
    def _load_instruments(self):
        ...
        if self.instruments_file.exists():
            try:
                ...
                logger.info(
                    f"Loaded {len(self.instruments)} instruments from {self.instruments_file}"
                )
            except Exception as e:
                logger.warning(
                    f"Failed to load instruments from {self.instruments_file}: {e}"
                )
...
class CurrencyConverter:
    ...
    def calculate_pip_value(
        ...
        quote_currency = (
            instrument.symbol[3:6] if len(instrument.symbol) >= 6 else "USD"
        )
